...
class App {
	...
	public function wp_rest_endpoints( $request ) {
		...
		register_rest_route(
			...
			[
				...
				'permission_callback' => [ $this, 'can_register_user' ],
			]
		);
	}

	/**
	 * Permission callback to check if user can register.
	 *
	 * @since 1.9.0
	 *
	 * @return bool
	...
	public function can_register_user() {
		return current_user_can( 'create_users' ) || ! is_user_logged_in();
	}
	...
	public function rest_registration_endpoint_handler( $request = null ) {
		...
		if ( empty( $username ) ) {
			$error->add( 'username_empty', esc_html__( 'Username field is required.', 'cariera-core' ), [ 'status' => 400 ] );
			return $error;
		}

		if ( username_exists( $username ) ) {
			$error->add( 'username_exists', esc_html__( 'This Username already exists', 'cariera-core' ), [ 'status' => 400 ] );
			return $error;
		}

		if ( ! validate_username( $username ) ) {
			$error->add( 'username_invalid', esc_html__( 'The Username you entered is not valid', 'cariera-core' ), [ 'status' => 400 ] );
			return $error;
		}

		// Error with the email field.
		if ( empty( $email ) ) {
			$error->add( 'email_empty', esc_html__( 'Email field is required.', 'cariera-core' ), [ 'status' => 400 ] );
			return $error;
		}

		if ( ! is_email( $email ) ) {
			$error->add( 'email_invalid', esc_html__( 'Email is not valid, please provide a correct email address.', 'cariera-core' ), [ 'status' => 400 ] );
			return $error;
		}

		if ( email_exists( $email ) ) {
			$error->add( 'email_exists', esc_html__( 'This Email already exists.', 'cariera-core' ), [ 'status' => 400 ] );
			return $error;
		}

		// Error when password is empty.
		if ( empty( $password ) ) {
			$error->add( 404, esc_html__( 'Password field is required.', 'cariera-core' ), [ 'status' => 400 ] );
			return $error;
		}
		...
		if ( empty( $role ) ) {
			$role = 'subscriber';
		} elseif ( $GLOBALS['wp_roles']->is_role( $role ) ) {
				// Silence is gold.
		} else {
			$error->add( 405, esc_html__( 'Role field is not valid. Check your User Roles from WordPress Dashboard.', 'cariera-core' ), [ 'status' => 400 ] );
			return $error;
		}
		...
		if ( ! is_wp_error( $user_id ) ) {
			...
			$response['code']    = 200;
			$response['message'] = esc_html__( 'User has been successful registered!', 'cariera-core' );
	...
	public function job_listing_rest_data( $data, $post, $context ) {
		...
		$data->data['meta']['job_application_url']           = '';
		$data->data['meta']['job_application_email']         = '';

		if ( class_exists( 'WP_Job_Manager_Applications' ) ) {
			$data->data['meta']['job_application_count'] = '';
		}
		...
		// Listing job application data.
		if ( strstr( $apply, '@' ) && is_email( $apply ) ) {
			$data->data['meta']['job_application_email'] = $post->_application;
		} else {
			$data->data['meta']['job_application_url'] = $post->_application;
		}

		// Job application count.
		if ( class_exists( 'WP_Job_Manager_Applications' ) ) {
			$data->data['meta']['job_application_count'] = get_job_application_count( $post->ID );
		}
