...
class Jobs_Extender extends Job_Manager {
	...
	public function job_listings_output( $atts ) {
		...
		// Jobs Layout.
		$jobs_layout         = ( 'list' === $atts['jobs_layout'] ) ? '_list' : '_' . $atts['jobs_layout'];
		$jobs_layout_wrapper = ( 'list' === $atts['jobs_layout'] ) ? 'job_list' : 'job_grid';
		$jobs_version        = ( 'list' === $atts['jobs_layout'] ) ? $atts['jobs_list_version'] : $atts['jobs_grid_version'];
		...
		if ( ! empty( $_GET['search_companies'] ) ) {
			$atts['companies']    = sanitize_text_field( wp_unslash( $_GET['search_companies'] ) );
			$disable_client_state = true;
		}
		...
		$atts['companies']          = is_array( $atts['companies'] ) ? array_filter( array_map( 'intval', $atts['companies'] ) ) : array_filter( array_map( 'intval', explode( ',', $atts['companies'] ) ) );
		...
		if ( $atts['show_filters'] ) {
			get_job_manager_template(
				...
				[
					...
					'companies'                 => $atts['companies'],
				]
			);

			get_job_manager_template( 'job-listings-start.php', [ 'jobs_layout_wrapper' => $jobs_layout_wrapper ] );
			get_job_manager_template( 'job-listings-end.php' );

			if ( ! $atts['show_pagination'] && $atts['show_more'] ) {
				echo '<a class="load_more_jobs" href="#" style="display:none;">' . esc_html__( 'Load more listings', 'cariera-core' ) . '</a>';
			}
		} else {
			...
			if ( $jobs->have_posts() || 1 === absint( $atts['sidebar_search'] ) ) {
				...
				if ( $jobs->found_posts > $atts['per_page'] && $atts['show_more'] ) {
					...
					if ( $atts['show_pagination'] ) {
						...
						echo '<a class="load_more_jobs" href="#">' . esc_html__( 'Load more listings', 'cariera-core' ) . '</a>';
					}
				}
			} else {
		...
		if ( ! empty( $atts['companies'] && get_option( 'cariera_company_manager_integration' ) ) ) {
			$data_attributes['companies'] = implode( ',', $atts['companies'] );
		}
	...
	public function get_listings_result( $result, $jobs ) {
		...
		$job_layout  = isset( $_POST['job_layout'] ) ? sanitize_text_field( wp_unslash( $_POST['job_layout'] ) ) : ''; // phpcs:ignore WordPress.Security.NonceVerification.Missing
		$job_version = isset( $_POST['job_version'] ) ? sanitize_text_field( wp_unslash( $_POST['job_version'] ) ) : ''; // phpcs:ignore WordPress.Security.NonceVerification.Missing

		// In case S&F for WPJM is activated.
		$job_layout  = isset( $_POST['data_params'], $_POST['data_params']['job_layout'] ) ? sanitize_text_field( wp_unslash( $_POST['data_params']['job_layout'] ) ) : $job_layout; // phpcs:ignore WordPress.Security.NonceVerification.Missing
		$job_version = isset( $_POST['data_params'], $_POST['data_params']['job_version'] ) ? sanitize_text_field( wp_unslash( $_POST['data_params']['job_version'] ) ) : $job_version; // phpcs:ignore WordPress.Security.NonceVerification.Missing
