...
class Resumes_Extender extends Resume_Manager {
	...
	public function output_resumes( $atts ) {
		...
		extract( // phpcs:ignore
			$atts = shortcode_atts( // phpcs:ignore
				apply_filters(
		...
		if ( ! is_null( $featured ) ) {
			$featured = ( is_bool( $featured ) && $featured ) || in_array( $featured, [ '1', 'true', 'yes' ], true ) ? true : false;
		}

		// phpcs:ignore
		if ( ! empty( $_GET['search_keywords'] ) ) {
			$keywords = sanitize_text_field( wp_unslash( $_GET['search_keywords'] ) ); // phpcs:ignore
		}

		// phpcs:ignore
		if ( ! empty( $_GET['search_location'] ) ) {
			$location = sanitize_text_field( wp_unslash( $_GET['search_location'] ) ); // phpcs:ignore
		}

		// phpcs:ignore
		if ( ! empty( $_GET['search_category'] ) ) {
			$selected_category = sanitize_text_field( wp_unslash( $_GET['search_category'] ) ); // phpcs:ignore
		}

		// phpcs:ignore
		if ( ! empty( $_GET['search_skills'] ) ) {
			$skills = sanitize_text_field( wp_unslash( $_GET['search_skills'] ) ); // phpcs:ignore
		}
		...
		if ( $show_filters ) {
			...
			if ( ! $show_pagination && $show_more ) {
				echo '<a class="load_more_resumes" href="#" style="display:none;"><strong>' . esc_html__( 'Load more resumes', 'cariera-core' ) . '</strong></a>';
			}
		} else {
			...
			if ( $resumes->have_posts() ) {
				...
				if ( $resumes->found_posts > $per_page && $show_more ) {
					...
					if ( $show_pagination ) {
						...
						<a class="load_more_resumes" href="#"><strong><?php esc_html_e( 'Load more resumes', 'cariera-core' ); ?></strong></a>
						<?php
					}
				}
			} else {
	...
	public function get_listings_result( $result, $resumes ) {
		...
		$resumes_layout  = isset( $_POST['resume_layout'] ) ? sanitize_text_field( wp_unslash( $_POST['resume_layout'] ) ) : ''; // phpcs:ignore
		$resumes_version = isset( $_POST['resume_version'] ) ? sanitize_text_field( wp_unslash( $_POST['resume_version'] ) ) : ''; // phpcs:ignore

		// In case S&F for WPJM is activated.
		$resumes_layout  = isset( $_POST['data_params'], $_POST['data_params']['resume_layout'] ) ? sanitize_text_field( wp_unslash( $_POST['data_params']['resume_layout'] ) ) : $resumes_layout; // phpcs:ignore
		$resumes_version = isset( $_POST['data_params'], $_POST['data_params']['resume_version'] ) ? sanitize_text_field( wp_unslash( $_POST['data_params']['resume_version'] ) ) : $resumes_version; // phpcs:ignore

		$search_location   = isset( $_POST['search_location'] ) ? sanitize_text_field( wp_unslash( $_POST['search_location'] ) ) : ''; // phpcs:ignore
		$search_keywords   = isset( $_POST['search_keywords'] ) ? sanitize_text_field( wp_unslash( $_POST['search_keywords'] ) ) : ''; // phpcs:ignore
		$search_categories = isset( $_POST['search_categories'] ) ? $_POST['search_categories'] : ''; // phpcs:ignore
		$search_skills     = isset( $_POST['search_skills'] ) ? sanitize_text_field( wp_unslash( $_POST['search_skills'] ) ) : ''; // phpcs:ignore
		...
		$args = [
			...
			'orderby'           => sanitize_text_field( $_POST['orderby'] ), // phpcs:ignore
			'order'             => sanitize_text_field( $_POST['order'] ), // phpcs:ignore
			'posts_per_page'    => absint( $_POST['per_page'] ), // phpcs:ignore
		];
		...
		if ( ! in_array( $_POST['orderby'], [ 'rand', 'rand_featured' ], true ) ) {
			$args['offset'] = ( absint( $_POST['page'] ) - 1 ) * absint( $_POST['per_page'] ); // phpcs:ignore
		}

		// phpcs:ignore
		if ( isset( $_POST['featured'] ) && ( $_POST['featured'] === 'true' || $_POST['featured'] === 'false' ) ) {
			$args['featured'] = $_POST['featured'] === 'true' ? true : false; // phpcs:ignore
		}
		...
		<?php else : ?>
			...
			<li class="no_resumes_found"><?php esc_html_e( 'No resumes found matching your selection.', 'cariera-core' ); ?></li>
		...
		*/
		...
		if ( $search_keywords || $search_location || $search_categories || apply_filters( 'resume_manager_get_resumes_custom_filter', false ) ) {
			...
			if ( $search_keywords ) {
				// translators: 1. Search keywords, 2. Categories (if any).
				$showing_resumes = sprintf( esc_html__( 'Showing &ldquo;%1$s&rdquo; %2$sresumes', 'cariera-core' ), $search_keywords, implode( ', ', $showing_categories ) );
			} else {
				// translators: 1. Categories (if any).
				$showing_resumes = sprintf( esc_html__( 'Showing all %sresumes', 'cariera-core' ), implode( ', ', $showing_categories ) . ' ' );
			}

			// translators: 1. Search location.
			$showing_location = $search_location ? sprintf( ' ' . esc_html__( 'located in &ldquo;%s&rdquo;', 'cariera-core' ), $search_location ) : '';
		...
		// Generate pagination.
		if ( isset( $_POST['show_pagination'] ) && $_POST['show_pagination'] === 'true' ) { // phpcs:ignore
			$result['pagination'] = get_job_listing_pagination( $resumes->max_num_pages, absint( $_POST['page'] ) ); // phpcs:ignore
		}
