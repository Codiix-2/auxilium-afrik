...
class Bookmarks {
	...
	public function bookmark_trigger() {
		...
		if ( is_user_logged_in() ) {
			echo '<a href="#bookmark-popup-' . esc_attr( get_the_ID() ) . '" class="company-bookmark popup-with-zoom-anim" aria-label="' . esc_attr__( 'Bookmark company', 'cariera-core' ) . '"><i class="lar la-heart"></i></a>';
		} else {
		...
		<div id="bookmark-popup-<?php echo esc_attr( get_the_ID() ); ?>" class="small-dialog zoom-anim-dialog mfp-hide">
			<div class="bookmarks-popup">
				<div class="small-dialog-headline">
					<h3 class="title"><?php esc_html_e( 'Bookmark Details', 'cariera-core' ); ?></h3>
				</div>
	...
	public function is_bookmarked( $post_id ) {
		...
		return $wpdb->get_var( $wpdb->prepare( "SELECT id FROM {$wpdb->prefix}job_manager_bookmarks WHERE post_id = %d AND user_id = %d;", $post_id, get_current_user_id() ) ) ? true : false; // phpcs:ignore
	}
	...
	public function bookmark_handler() {
		...
		if ( ! empty( $_POST['submit_bookmark'] ) ) {
			// phpcs:ignore
			if ( ! wp_verify_nonce( wp_unslash( $_REQUEST['_wpnonce'] ), 'update_bookmark' ) ) {
				$action_data = [
					'error_code' => 400,
					'error'      => esc_html__( 'Bad request', 'cariera-core' ),
				];
			} else {
				$post_id = absint( $_POST['bookmark_post_id'] ); // phpcs:ignore
				$note    = wp_kses_post( stripslashes( $_POST['bookmark_notes'] ) ); // phpcs:ignore
		...
		if ( ! empty( $_GET['remove_bookmark'] ) ) {
			// phpcs:ignore
			if ( ! wp_verify_nonce( wp_unslash( $_REQUEST['_wpnonce'] ), 'remove_bookmark' ) ) {
				$action_data = [
					'error_code' => 400,
					'error'      => esc_html__( 'Bad request', 'cariera-core' ),
				];
