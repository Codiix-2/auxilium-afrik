...
// Ensure the base class exists before extending it.
if ( ! class_exists( 'WP_Job_Manager_Form' ) && defined( 'JOB_MANAGER_PLUGIN_DIR' ) ) {
...
class Submit_Company extends \WP_Job_Manager_Form {
	...
	public function __construct() {
		...
		if ( $this->use_agreement_checkbox() ) {
			...
			add_filter( 'cariera_submit_company_form_validate_fields', [ $this, 'validate_agreement_checkbox' ] );
		}
		...
		if ( class_exists( '\WP_Job_Manager\WP_Job_Manager_Recaptcha' ) ) {
			\WP_Job_Manager\WP_Job_Manager_Recaptcha::instance()->maybe_enable_recaptcha(
				'cariera_enable_recaptcha_company_submission',
				[ 'submit_company_form_company_fields_end' ],
				[ 'cariera_submit_company_form_validate_fields' ]
			);
		} else {
			...
			add_action( 'cariera_submit_company_form_validate_fields', [ $this, 'validate_recaptcha_field' ] );
		...
		$this->steps = (array) apply_filters(
			...
			[
				'submit'  => [
					'name'     => esc_html__( 'Submit Details', 'cariera-core' ),
					'view'     => [ $this, 'submit' ],
					'handler'  => [ $this, 'submit_handler' ],
					'priority' => 10,
				],
				'preview' => [
					'name'     => esc_html__( 'Preview', 'cariera-core' ),
					'view'     => [ $this, 'preview' ],
					'handler'  => [ $this, 'preview_handler' ],
					'priority' => 20,
				],
				'done'    => [
					'name'     => esc_html__( 'Done', 'cariera-core' ),
					'view'     => [ $this, 'done' ],
					'handler'  => '',
					'priority' => 30,
				],
		...
		if ( $this->company_id ) {
			...
			} elseif ( 0 === $this->step && ! in_array( $company_status, apply_filters( 'cariera_valid_submit_company_statuses', [ 'preview' ] ), true ) ) {
				$this->company_id = 0;
				$this->job_id     = 0;
				$this->step       = 0;
			}
	...
	public function init_fields() {
		...
		$this->fields = apply_filters(
			...
			[
				'company_fields' => [
					'company_name'         => [
						'label'       => esc_html__( 'Company Name', 'cariera-core' ),
						'type'        => 'text',
						'required'    => true,
						'placeholder' => esc_html__( 'The full Company name', 'cariera-core' ),
						'priority'    => 1,
					],
					'company_website'      => [
						'label'       => esc_html__( 'Company Website', 'cariera-core' ),
						'type'        => 'text',
						'required'    => false,
						'placeholder' => esc_html__( 'https://domain.com', 'cariera-core' ),
						'priority'    => 2,
					],
					'company_email'        => [
						'label'       => esc_html__( 'Company Email', 'cariera-core' ),
						'type'        => 'text',
						'required'    => false,
					...
					'company_phone'        => [
						'label'       => esc_html__( 'Company Contact Phone', 'cariera-core' ),
						'type'        => 'text',
						'required'    => false,
						'placeholder' => esc_html__( '+1 (202) 555 0183', 'cariera-core' ),
						'priority'    => 2,
					],
					'company_location'     => [
						...
						'type'        => 'text',
						'required'    => false,
						'placeholder' => esc_html__( 'e.g. "London, UK"', 'cariera-core' ),
						'priority'    => 3,
					],
					'company_since'        => [
						...
						'type'        => 'date',
						'required'    => false,
						'placeholder' => esc_html__( 'Established date/year', 'cariera-core' ),
						'priority'    => 3,
					],
					'company_category'     => [
					...
					'company_team_size'    => [
						'label'       => esc_html__( 'Company team size', 'cariera-core' ),
						'type'        => 'term-select',
						'taxonomy'    => 'company_team_size',
						'required'    => true,
						'placeholder' => esc_html__( 'Choose your company\'s team size', 'cariera-core' ),
						'default'     => '',
						'priority'    => 4,
					],
					'company_content'      => [
						'label'       => esc_html__( 'Company Description', 'cariera-core' ),
						'type'        => 'wp-editor',
						'required'    => true,
						'placeholder' => '',
						'priority'    => 5,
					...
					'company_video'        => [
						...
						'required'    => false,
						'priority'    => 6,
						'placeholder' => esc_html__( 'A link to a video about the company', 'cariera-core' ),
					],
					'company_facebook'     => [
						...
						'required'    => false,
						'priority'    => 6,
						'placeholder' => esc_html__( 'Facebook Page URL', 'cariera-core' ),
					],
					'company_twitter'      => [
						'label'       => esc_html__( 'X', 'cariera-core' ),
						'type'        => 'text',
						'required'    => false,
						'priority'    => 6,
						'placeholder' => esc_html__( 'X Page URL', 'cariera-core' ),
					],
					'company_linkedin'     => [
						...
						'required'    => false,
						'priority'    => 6,
						'placeholder' => esc_html__( 'LinkedIn Page URL', 'cariera-core' ),
					],
					'company_instagram'    => [
						...
						'required'    => false,
						'priority'    => 6,
						'placeholder' => esc_html__( 'Instagram Page URL', 'cariera-core' ),
					],
					'company_logo'         => [
	 ...
	 *
	 * @param mixed $values
	 * @throws \Exception When fields are invalid.
	 */
	public function validate_fields( $values ) {
		foreach ( $this->fields as $group_key => $fields ) {
			foreach ( $fields as $key => $field ) {
				if ( $field['required'] && $this->is_empty( $values[ $group_key ][ $key ] ) ) {
					// translators: %s is label of the field.
					return new \WP_Error( 'validation-error', sprintf( esc_html__( '%s is a required field', 'cariera-core' ), $field['label'] ) );
				}
					...
					if ( is_array( $values[ $group_key ][ $key ] ) ) {
						foreach ( $values[ $group_key ][ $key ] as $term ) {
							if ( ! term_exists( $term, $field['taxonomy'] ) ) {
								// translators: %s is label of the field.
								return new \WP_Error( 'validation-error', sprintf( esc_html__( '%s is invalid', 'cariera-core' ), $field['label'] ) );
							}
						}
				...
				if ( 'company_email' === $key ) {
					if ( ! empty( $values[ $group_key ][ $key ] ) && ! is_email( $values[ $group_key ][ $key ] ) ) {
						throw new \Exception( esc_html__( 'Please enter a valid email address', 'cariera-core' ) );
					}
				}
				...
				if ( 'file' === $field['type'] ) {
					...
					if ( ! empty( $check_value ) ) {
						foreach ( $check_value as $file_url ) {
							...
							if ( empty( $file_url ) ) {
								throw new \Exception( esc_html__( 'Invalid attachment provided.', 'cariera-core' ) );
							}
						}
					}
				}
	...
	public function submit() {
		...
		if ( $this->company_id ) {
			...
			// Get user meta.
		} elseif ( is_user_logged_in() && empty( $_POST['submit_company'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Missing -- Safe input.
		...
		if ( $total_companies < $submission_limit || ! $submission_limit ) {
			get_job_manager_template(
				...
				[
					...
					'step'               => $this->get_step(),
					'submit_button_text' => apply_filters( 'cariera_submit_company_form_submit_button_text', esc_html__( 'Preview Company', 'cariera-core' ) ),
				],
				'wp-job-manager-companies'
			);
		} else {
	...
	public function submit_handler() {
		try {
			...
			if ( ! is_user_logged_in() ) {
				...
				if ( cariera_enable_registration() ) {
					if ( cariera_company_manager_user_requires_account() ) {
						if ( ! cariera_company_manager_generate_username_from_email() && empty( $input_create_account_username ) ) {
							throw new \Exception( esc_html__( 'Please enter a username.', 'cariera-core' ) );
						}

						if ( ! cariera_company_use_standard_password_setup_email() ) {
							if ( empty( $input_create_account_password ) ) {
								throw new \Exception( esc_html__( 'Please enter a password.', 'cariera-core' ) );
							}
						}

						if ( empty( $input_create_account_email ) ) {
							throw new \Exception( esc_html__( 'Please enter your company email address.', 'cariera-core' ) );
						}
					}

					if ( ! cariera_company_use_standard_password_setup_email() && ! empty( $input_create_account_password ) ) {
						if ( empty( $input_create_account_password_verify ) || $input_create_account_password_verify !== $input_create_account_password ) {
							throw new \Exception( esc_html__( 'Passwords must match.', 'cariera-core' ) );
						}
						if ( ! wpjm_validate_new_password( sanitize_text_field( wp_unslash( $input_create_account_password ) ) ) ) {
							...
							if ( $password_hint ) {
								...
								throw new \Exception( sprintf( esc_html__( 'Invalid Password: %s', 'cariera-core' ), $password_hint ) );
							} else {
								throw new \Exception( esc_html__( 'Password is not valid.', 'cariera-core' ) );
							}
						}
					}
					...
					if ( ! empty( $input_create_account_email ) ) {
						if ( version_compare( JOB_MANAGER_VERSION, '1.20.0', '<' ) ) {
							...
							$create_account = wp_job_manager_create_account(
								[
									'username' => ( cariera_company_manager_generate_username_from_email() || empty( $input_create_account_username ) ) ? '' : $input_create_account_username,
									'password' => ( cariera_company_use_standard_password_setup_email() || empty( $input_create_account_password ) ) ? '' : $input_create_account_password,
			...
			if ( cariera_company_manager_user_requires_account() && ! is_user_logged_in() ) {
				throw new \Exception( esc_html__( 'You must be signed in to post your company.', 'cariera-core' ) );
			}
	...
	public function update_company_data( $values ) {
		...
		// Handle attachments.
		if ( count( $maybe_attach ) && apply_filters( 'cariera_company_attach_uploaded_files', false ) ) {
	...
	public function use_recaptcha_field() {
		$recaptcha_available = $this->are_recaptcha_methods_available();
	 ...
	 * @return bool
	 */
	private function are_recaptcha_methods_available() {
		if ( class_exists( '\WP_Job_Manager\WP_Job_Manager_Recaptcha' ) ) {
			$recaptcha_instance = \WP_Job_Manager\WP_Job_Manager_Recaptcha::instance();
			return $recaptcha_instance->is_recaptcha_available();
		} else {
			return $this->is_recaptcha_available();
		}
		return false;
	}
