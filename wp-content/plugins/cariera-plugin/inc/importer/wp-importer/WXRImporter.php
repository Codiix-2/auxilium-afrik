 ...
 * @version 1.9.4
 *
 * @see https://github.com/proteusthemes/WordPress-Importer/blob/master/src/WXRImporter.php
 */
...
class WXRImporter extends \WP_Importer {
	...
	protected function get_reader( $file ) {
		...
		if ( ! $status ) {
			return new \WP_Error( 'wxr_importer.cannot_parse', __( 'Could not open the file for parsing', 'cariera-core' ) );
		}
		...
		while ( $reader->read() ) {
			if ( \XMLReader::PI == $reader->nodeType && 'WXR_Importer' === $reader->localName ) {
				if ( preg_match( "/^namespace-uri='([^']+)'\s+plugin-name='([^']+)'\s+plugin-slug='([^']+)'\s+plugin-url='([^']+)'\s+\$/", $reader->value, $matches ) > 0 ) {
					$data->extension_namespaces[ $matches[1] ][] = [
						'name' => $matches[2],
						'slug' => $matches[3],
						'url'  => $matches[4],
					];
				...
				switch ( $reader->localName ) {
					case 'rss':
						...
						if ( empty( $this->version ) ) {
							return new \WP_Error(
								'wxr_importer.unknown_version',
								__( 'This does not appear to be a WXR file, missing WXR version number.', 'cariera-core' )
							);
						}
						elseif ( version_compare( $this->version, self::MAX_WXR_VERSION, '>' ) ) {
							$this->logger->warning(
								sprintf(
									__( 'This WXR file (version %1$s) is newer than the importer (version %2$s) and may not be supported. Please consider updating.', 'cariera-core' ),
									$this->version,
		...
		foreach ( $node->childNodes as $child ) {
			...
			elseif ( is_null( $child->namespaceURI ) ) {
				...
				if ( ! isset( $extension_elements[ $child->namespaceURI ] ) ) {
					$extension_elements[ $child->namespaceURI ] = [];
				}
			...
			switch ( $child->localName ) {
				...
				case 'termmeta':
					...
					foreach ( $child->childNodes as $meta_child ) {
						if ( $meta_child->nodeType !== XML_ELEMENT_NODE ) {
							continue;
						}

						if ( self::WXR_NAMESPACE_URI !== $meta_child->namespaceURI ) {
							continue;
						}

						if ( $meta_child->localName === 'meta_key' ) {
							$key = $meta_child->textContent;
						}
						elseif ( $meta_child->localName === 'meta_value' ) {
							$value = $meta_child->textContent;
						}
					}
					if ( $key !== '' ) {
						$meta[] = [
	...
	protected function import_start( $file ) {
		if ( ! is_file( $file ) ) {
			return new \WP_Error( 'wxr_importer.file_missing', __( 'The file does not exist, please try again.', 'cariera-core' ) );
		}
		...
		require_once __DIR__ . '/wxr-transform.php';
		$transform_wxr = new \Transform_WXR();
		if ( is_wp_error( $transform_wxr ) ) {
			$this->log_error( __( 'Could not create WXR transformer.', 'cariera-core' ) );
	...
	public function set_user_mapping( $mapping ) {
		foreach ( $mapping as $map ) {
			if ( empty( $map['old_slug'] ) || empty( $map['old_id'] ) || empty( $map['new_id'] ) ) {
				$this->logger->warning( __( 'Invalid author mapping', 'cariera-core' ) );
	...
	protected function process_attachment( $post, $meta, $remote_url ) {
		...
		if ( ! $info ) {
			return new \WP_Error( 'attachment_processing_error', __( 'Invalid file type', 'cariera-core' ) );
		}
	...
	protected function process_comments( $comments, $post_id, $post, $post_exists = false ) {
		...
		usort( $comments, [ $this, 'sort_comments_by_id' ] );
		...
		foreach ( $comments as $key => $comment ) {
			...
			$meta = isset( $comment['commentmeta'] ) ? $comment['commentmeta'] : [];
	...
	protected function post_process_menu_item( $post_id ) {
		...
		if ( ! empty( $menu_object ) ) {
			...
			$this->logger->warning(
				sprintf(
					__( 'Could not find the menu object for "%1$s" (post #%2$d)', 'cariera-core' ),
			...
			$this->logger->debug(
				sprintf(
					__( 'Post %1$d was imported with object "%2$d" of type "%3$s", but could not be found', 'cariera-core' ),
					$post_id,
					$menu_object_id,
					$menu_item_type
				)
	...
	protected function process_post( $data, $meta, $comments, $terms ) {
		...
		if ( ! $post_type_object ) {
			$this->logger->warning(
				sprintf(
					__( 'Failed to import "%1$s": Invalid post type %2$s', 'cariera-core' ),
					$data['post_title'],
					$data['post_type']
				)
			);
		...
		if ( $post_exists ) {
			$this->logger->info(
				sprintf(
					__( '%1$s "%2$s" already exists.', 'cariera-core' ),
		...
		if ( is_wp_error( $post_id ) ) {
			$this->logger->error(
				sprintf(
					__( 'Failed to import "%1$s" (%2$s)', 'cariera-core' ),
					$data['post_title'],
					$post_type_object->labels->singular_name
				)
			);
		...
		$this->logger->info(
			sprintf(
				__( 'Imported "%1$s" (%2$s)', 'cariera-core' ),
				$data['post_title'],
				$post_type_object->labels->singular_name
			)
		);
		$this->logger->debug(
			sprintf(
				__( 'Post %1$d remapped to %2$d', 'cariera-core' ),
				$original_id,
				$post_id
			)
		);
	...
	protected function process_term( $data, $meta ) {
		...
		if ( $existing ) {
			$this->logger->notice(
				sprintf(
					__( 'Term "%1$s" (%2$s) already exists', 'cariera-core' ),
					$data['name'],
					$data['taxonomy']
				)
			);
			...
			// Process meta even for existing terms.
			$this->process_term_meta( $meta, $existing, $data );
		...
		$this->logger->debug(
			sprintf(
				__( 'Term %1$d remapped to %2$d', 'cariera-core' ),
				$original_id,
				$term_id
			)
		);
	...
	protected function process_user( $data, $meta ) {
		...
		if ( isset( $this->mapping['user'][ $original_id ] ) ) {
			...
			$this->logger->notice(
				sprintf(
					__( 'Skipped importing user "%s"', 'cariera-core' ),
					$data['user_login']
				)
			);
		...
		if ( is_wp_error( $user_id ) ) {
			$this->logger->error(
				sprintf(
					__( 'Failed to import user "%s"', 'cariera-core' ),
					$userdata['user_login']
				)
			);
		...
		$this->logger->info(
			sprintf(
				__( 'Imported user "%s"', 'cariera-core' ),
				$userdata['user_login']
			)
		);
		$this->logger->debug(
			sprintf(
				__( 'User %1$d remapped to %2$d', 'cariera-core' ),
				$original_id,
				$user_id
			)
		);
	...
	protected function process_link( $data, $link_categories ) {
		...
		if ( $link_exists ) {
			$this->logger->notice(
				sprintf(
					__( 'Link "%s" already exists.', 'cariera-core' ),
					$data['link_name']
				)
			);
		...
		if ( is_wp_error( $link_id ) ) {
			$this->logger->error(
				sprintf(
					__( 'Failed to import link "%s"', 'cariera-core' ),
					$data['link_name']
				)
			);
		...
		$this->logger->info(
			sprintf(
				__( 'Imported link "%s"', 'cariera-core' ),
				$data['link_name']
			)
		);
		$this->logger->debug(
			sprintf(
				__( 'Link %1$d remapped to %2$d', 'cariera-core' ),
				$original_id,
				$link_id
			)
		);
	...
	protected function fetch_remote_file( $url, $post ) {
		...
		if ( $code !== 200 ) {
			...
			return new \WP_Error(
				'import_file_error',
				sprintf(
					__( 'Remote server returned %1$d %2$s for %3$s', 'cariera-core' ),
					$code,
		...
		if ( isset( $headers['content-length'] ) && $filesize !== (int) $headers['content-length'] ) {
			unlink( $upload['file'] );
			return new \WP_Error( 'import_file_error', __( 'Remote file is incorrect size', 'cariera-core' ) );
		}

		if ( 0 === $filesize ) {
			unlink( $upload['file'] );
			return new \WP_Error( 'import_file_error', __( 'Zero size file downloaded', 'cariera-core' ) );
		}

		$max_size = (int) $this->max_attachment_size();
		if ( ! empty( $max_size ) && $filesize > $max_size ) {
			unlink( $upload['file'] );
			$message = sprintf( __( 'Remote file is too large, limit is %s', 'cariera-core' ), size_format( $max_size ) );
		...
		foreach ( $todo as $post_id => $_ ) {
			$this->logger->debug(
				...
				// for when debug logging is off
					__( 'Running post-processing for post %d', 'cariera-core' ),
					$post_id
				)
			);
			...
			if ( ! empty( $parent_id ) ) {
				...
				if ( isset( $this->mapping['post'][ $parent_id ] ) ) {
					...
					$this->logger->warning(
						sprintf(
							__( 'Could not find the post parent for "%1$s" (post #%2$d)', 'cariera-core' ),
							get_the_title( $post_id ),
							$post_id
						)
					);
					$this->logger->debug(
						sprintf(
							__( 'Post %1$d was imported with parent %2$d, but could not be found', 'cariera-core' ),
							$post_id,
							$parent_id
						)
					);
			...
			if ( ! empty( $author_slug ) ) {
				...
				if ( isset( $this->mapping['user_slug'][ $author_slug ] ) ) {
					...
					$this->logger->warning(
						sprintf(
							__( 'Could not find the author for "%1$s" (post #%2$d)', 'cariera-core' ),
							get_the_title( $post_id ),
							$post_id
						)
					);
					$this->logger->debug(
						sprintf(
							__( 'Post %1$d was imported with author "%2$s", but could not be found', 'cariera-core' ),
							$post_id,
							$author_slug
						)
					);
			...
			if ( empty( $data ) ) {
				$this->logger->debug(
					sprintf(
						__( 'Post %d was marked for post-processing, but none was required.', 'cariera-core' ),
						$post_id
					)
				);
				continue;
			...
			if ( is_wp_error( $result ) ) {
				$this->logger->warning(
					sprintf(
						__( 'Could not update "%1$s" (post #%2$d) with mapped data', 'cariera-core' ),
						get_the_title( $post_id ),
						$post_id
					)
				);
		...
		foreach ( $todo as $term_id => $_ ) {
			...
			if ( ! empty( $parent_term_slug ) ) {
				...
				elseif ( $parent_id = $this->term_exists(
					...
					$this->logger->warning(
						sprintf(
							__( 'Could not find the parent for term #%d', 'cariera-core' ),
							$term_id,
							$parent_term_slug
						)
					);
			...
			if ( empty( $result ) ) {
				$this->logger->warning(
					sprintf(
						__( 'Cound not update term #%d with mapped data', 'cariera-core' ),
						$term_id
					)
				);
				continue;
		...
		foreach ( $todo as $comment_id => $_ ) {
			...
			if ( ! empty( $parent_id ) ) {
				...
				if ( isset( $this->mapping['comment'][ $parent_id ] ) ) {
					...
					$this->logger->warning(
						sprintf(
							__( 'Could not find the comment parent for comment #%d', 'cariera-core' ),
							$comment_id
						)
					);
					$this->logger->debug(
						sprintf(
							__( 'Comment %1$d was imported with parent %2$d, but could not be found', 'cariera-core' ),
							$comment_id,
							$parent_id
						)
					);
			...
			if ( ! empty( $author_id ) ) {
				...
				if ( isset( $this->mapping['user'][ $author_id ] ) ) {
					...
					$this->logger->warning(
						sprintf(
							__( 'Could not find the author for comment #%d', 'cariera-core' ),
							$comment_id
						)
					);
					$this->logger->debug(
						sprintf(
							__( 'Comment %1$d was imported with author %2$d, but could not be found', 'cariera-core' ),
							$comment_id,
							$author_id
						)
					);
			...
			if ( empty( $result ) ) {
				$this->logger->warning(
					sprintf(
						__( 'Could not update comment #%d with mapped data', 'cariera-core' ),
						$comment_id
					)
				);
				continue;
	...
	protected function post_process_links( $todo ) {
		...
		foreach ( $todo as $link_id => $_ ) {
			$this->logger->debug(
				...
				// Note: title intentionally not used to skip extra processing
				// for when debug logging is off
					__( 'Running post-processing for link %d', 'cariera-core' ),
					$link_id
				)
			);
			...
			if ( ! empty( $author_slug ) ) {
				...
				if ( isset( $this->mapping['user_slug'][ $author_slug ] ) ) {
					...
					$this->logger->warning(
						sprintf(
							__( 'Could not find the author for "%1$s" (link #%2$d)', 'cariera-core' ),
							$link_name,
							$link_id
						)
					);
					$this->logger->debug(
						sprintf(
							__( 'Link %1$d was imported with author "%2$s", but could not be found', 'cariera-core' ),
							$link_id,
							$author_slug
						)
					);
			...
			if ( empty( $data ) ) {
				$this->logger->debug(
					sprintf(
						__( 'Link %d was marked for post-processing, but none was required.', 'cariera-core' ),
						$link_id
					)
				);
				continue;
			...
			if ( is_wp_error( $result ) ) {
				...
				$this->logger->warning(
					sprintf(
						__( 'Could not update "%1$s" (link #%2$d) with mapped data', 'cariera-core' ),
						$link_name,
						$link_id
					)
				);
