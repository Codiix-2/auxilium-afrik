...
class Transform_WXR {
	...
	function startElement( $reader, $writer, $parent ) {
		if ( $this->is_wxr_namespaceURI( $reader->namespaceURI, true ) ) {
			...
			if ( in_array( $reader->localName, [ 'wxr_version', 'base_blog_url' ] ) ) {
		...
		while ( $reader->read() ) {
			switch ( $reader->nodeType ) {
				...
				case \XMLReader::END_ELEMENT:
					if ( $this->is_wxr_namespaceURI( $reader->namespaceURI, true ) ) {
						if ( in_array( $reader->localName, [ 'tag', 'category' ] ) ) {
	...
	function map_localName( $reader, $parent ) {
		...
		elseif ( in_array( $reader->localName, [ 'termmeta', 'postmeta', 'commentmeta' ] ) ) {
			return 'meta';
		}
		...
		if ( ( in_array(
			...
			[ 'author_', 'post_', 'meta_', 'comment_', 'base_' ]
		) &&
						'comment_status' !== $reader->localName ) ||
				( $this->is_wxr_namespaceURI( $parent['namespaceURI'] ) &&
					in_array( $parent['localName'], [ 'tag', 'category', 'term' ] ) ) ) {
