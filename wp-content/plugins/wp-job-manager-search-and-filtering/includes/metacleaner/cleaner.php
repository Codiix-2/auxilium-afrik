...
class Cleaner {
	...
	public function get_data() {
		...
		if( ! class_exists( $class ) ){
			wp_send_json_error( __( 'Unable to find correct class, please try again or contact support!', 'wp-job-manager-search-and-filtering' ) );
		}
	...
	public function handle_count() {
		...
		if ( ! isset( $_POST['meta_keys'] ) || empty( $_POST['meta_keys'] ) ) {
			wp_send_json_error( __( 'You must specify meta keys to search for in the database!', 'wp-job-manager-search-and-filtering' ) );
		}
		...
		if ( empty( $counts ) || empty( $counts['results'] ) ) {
			wp_send_json_error( __( 'No empty meta values found in database!  You are good to go!', 'wp-job-manager-search-and-filtering' ) );
		}
	...
	public function handle_cleanup() {
		...
		if( ! isset( $_POST['meta_db_ids'] ) ){
			wp_send_json_error( __( 'Meta db IDs are required!', 'wp-job-manager-search-and-filtering' ) );
		}
	...
	public function cleaner_output( $that, $settings ) {
		...
		if( ! current_user_can( 'manage_options' ) ){
			echo '<strong>' . __( 'You do not have high enough permissions to perform empty meta cleanup (for existing listings) in the database.  Please use an account that has "manage_options" capabilities/permissions.', 'wp-job-manager-search-and-filtering' ) . '</strong>';
			return;
		}
		...
		if( ! isset( $_GET['empty_meta_cleanup'] ) ){
			...
			echo "<a class=\"button button-primary\" href=\"{$url}\">" . __( 'Show Existing Meta Cleaner', 'wp-job-manager-search-and-filtering' ) . '</a>';
			echo '<p class="description">' . __( 'To prevent excessive queries and load time on settings page load, the existing meta cleaner is only initialized when you click the button above.', 'wp-job-manager-search-and-filtering' ) . '</p>';
			return;
		}
