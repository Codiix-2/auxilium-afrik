...
class sMyles_Updater_v2_Licenses {
	...
	public function death_to_sloppy_devs( $hook ) {
		...
		$scripts = array(
			...
			'jquery-ui-core',
			'jquery-ui-accordion',
			//			'jquery-ui-sortable', - sortable used for options sorting
			'jquery-ui-button',
			'wpum-admin-js',
			'scporderjs',
			'kwayyhs-custom-js',
			...
			'service_finder-js-job-apply', // Service Finder Theme
	...
	function ajax_deactivate(){
		...
		if( ! isset($_POST['slug']) ) wp_die( json_encode( array( 'error_title' => __( 'Error', 'wp-job-manager-search-and-filtering' ), 'error_msg' => __( 'Unable to determine slug to deactivate license.', 'wp-job-manager-search-and-filtering' ) ) ) );
		...
		try {
			...
			if( empty($plugin_product_id) ) {
				throw new Exception( __( 'Error determining the plugin Product ID.', 'wp-job-manager-search-and-filtering' ) );
			}

			if( empty($license_key) ){
				throw new Exception( __( 'Error pulling the license key from option data.', 'wp-job-manager-search-and-filtering' ) );
			}

			if( empty($email) ) {
				throw new Exception( __( 'Error pulling the email associated with the license key from option data.', 'wp-job-manager-search-and-filtering' ) );
			}

			if( empty($instance_id) ) {
				throw new Exception( __( 'Error pulling the instance ID associated with the license key from option data.', 'wp-job-manager-search-and-filtering' ) );
			}
			...
			if( FALSE === $deactivate_results ) {
				throw new Exception( __( 'Connection failed to the License Key API server - possible server issue.', 'wp-job-manager-search-and-filtering' ) );
				...
				$results = array('success_title'    => $plugin_product_id . __( ' Successfully Deactivated!', 'wp-job-manager-search-and-filtering' ),
								 'success_msg'      => sprintf( __( 'You have successfully deactivated the %s license.', 'wp-job-manager-search-and-filtering' ), $plugin_product_id ),
			...
			$results = array('error_title' => __( 'Error!', 'wp-job-manager-search-and-filtering' ), 'error_msg' => $e->getMessage() . '<br/>' . sprintf( __( 'The license has been successfully deactivated locally, but please visit your sMyles Plugins <a href="%s" target="_blank">My Account</a> page to verify this activation has been removed from our server.  There was an error automatically removing it, and chances are that is because it was already removed via your <a href="%s" target="_blank">My Account</a> page already..', 'wp-job-manager-search-and-filtering' ), 'https://plugins.smyl.es/my-account/' ) );
	...
	function ajax_activate() {
		...
		if( ! isset($_POST['slug'], $_POST['license_key'], $_POST['email']) ) {
			wp_die( json_encode( array(
										 'error_title' => __( 'Error', 'wp-job-manager-search-and-filtering' ),
										 'error_msg'   => __( 'Unable to determine slug, license key, and/or email to activate license, contact support.', 'wp-job-manager-search-and-filtering' ),
								 ) ) );
		}
		...
		try {
			...
			if( FALSE === $activate_results ) {
				...
				$results = array( 'success_title' => $plugin_product_id . __( ' Successfully Activated!', 'wp-job-manager-search-and-filtering' ), 'success_msg' => sprintf( __( 'You have successfully activated the %s license, and will now be able to automatically upgrade this plugin through WordPress when an update is available.', 'wp-job-manager-search-and-filtering' ), $plugin_product_id ), 'activate_results' => $activate_results );
				$known_plugins = $this->get_known_plugins();
			...
			$results = array('error_title' => __( 'Error!', 'wp-job-manager-search-and-filtering' ), 'error_msg' => $e->getMessage(), 'activate_results' => $activate_results );
	...
	function get_known_plugins(){
		...
		$plugins = apply_filters( 'smyles_updater_v2_known_plugins',
								  array(
										  ...
										  'wp-company-manager'   => array(
												  'title'      => 'WP Company Manager for WP Job Manager',
												  'class'      => 'WP_Company_Manager',
												  'product_id' => 'COMPANY_MANAGER_PROD_ID',
												  'version'    => 'COMPANY_MANAGER_VERSION'
										  ),
										  'wp-job-manager-search-and-filtering'   => array(
												  'title'      => 'Search and Filtering for WP Job Manager',
												  'class'      => 'WPJM_Search_Filtering',
												  'product_id' => 'WPJM_SEARCH_FILTERING_PROD_ID',
												  'version'    => 'WPJM_SEARCH_FILTERING_VERSION'
										  ),
								  )
		);
	...
	function licenses_page_output() {
		...
		<div id="smylesv2-modal" class="ui small basic modal" data-slug="">
			<div class="ui icon header">
				...
				<?php printf( __('Deactivate the %s License?', 'wp-job-manager-search-and-filtering' ), '<span id="smylesv2-modal-slug" style="color: #db2828;"></span>' ); ?>
			</div>
			<div class="actions">
				<div class="ui red basic cancel inverted button">
					<i class="remove icon"></i>
					<?php _e('No', 'wp-job-manager-search-and-filtering' ); ?>
				</div>
				<div class="ui green ok inverted button">
					<i class="checkmark icon"></i>
					<?php _e('Yes', 'wp-job-manager-search-and-filtering' ); ?>
				</div>
			</div>
		</div>
		<div class="wrap">
			<div id="icon-tools" class="icon32"></div>
			<h2><?php _e('sMyles Plugins Licensing', 'wp-job-manager-search-and-filtering' ); ?></h2>
			<h4 class="ui horizontal divider header">
				<i class="wordpress icon"></i>
				<?php _e('Plugins', 'wp-job-manager-search-and-filtering' ); ?>
			</h4>
