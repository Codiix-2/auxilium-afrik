...
class sMyles_Updater_v2 {
	...
	public function init_updates( $file ) {
		...
		register_activation_hook( $this->plugin_name, array($this, 'plugin_activation') );
		register_deactivation_hook( $this->plugin_name, array($this, 'plugin_deactivation') );

		add_action( 'admin_init', array( $this, 'admin_init' ), 0 );

		include_once __DIR__ . '/smyles-updater-api.php';
		include_once __DIR__ . '/smyles-updater-key-api.php';
		include_once __DIR__ . '/smyles-updater-licenses.php';
	...
	public function admin_init() {
		...
		if( empty($this->instance_id) ) {
			...
			if( ! class_exists( 'sMyles_Updater_Password_Management' ) ) {
				include_once __DIR__ . '/smyles-updater-passwords.php';
			}
	...
	public function error_notices() {
		...
		if( ! empty($this->errors) ) {
			foreach( $this->errors as $key => $error ) {
				include __DIR__ . '/views/html-error-notice.php';
	...
	public function handle_errors( $errors ) {
		...
		if( ! empty($errors['no_key']) ) {
			...
			$no_activation = $errors['no_activation'] != 'no_activation' ? $errors['no_activation'] : sprintf( 'No activations exist for %s on this website.  This could be due the plugin not being activated yet, the activation was removed via your My Account page on sMyles Plugins, you installed/activated the plugin on another site, you changed your website URL, or you have moved your site to a new server.  Please reactivate to receive upgrades and support.', esc_html( $this->plugin_data['Name'] ) );
			$this->deactivate_licence();
	...
	public function multisite_updates( $file, $plugin ) {
		...
		if( ! empty($update_cache->response[ $this->plugin_name ]) && version_compare( $this->plugin_data['Version'], $version_info->new_version, '<' ) ) {
			...
			include __DIR__ . '/views/html-ms-update.php';
...
//		delete_option( $this->plugin_slug . '_hide_key_notice' );
		...
		if( ! $this->api_key && sizeof( $this->errors ) === 0 && ! get_option( $this->plugin_slug . '_hide_key_notice' ) ) {
			include __DIR__ . '/views/html-key-notice.php';
		}
	}
	...
	public function activated_key_notice() {
		include __DIR__ . '/views/html-activated-key.php';
	}
	...
	public function deactivated_key_notice() {
		...
		include __DIR__ . '/views/html-deactivated-key.php';
	}
	...
	public function key_input( $file ) {
		...
		if( strtolower( basename( dirname( $file ) ) ) === strtolower( $this->plugin_slug ) ) {
			include __DIR__ . '/views/html-key-input.php';
		}
	}
}
