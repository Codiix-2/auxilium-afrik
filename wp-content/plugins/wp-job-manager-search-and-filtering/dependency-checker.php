...
/**
 * Class WPJMSF_Dependency_Checker
 *
 * @since 0.1.1
 *
 */
class WPJMSF_Dependency_Checker {
	...
	public static function check_dependencies() {
		...
		if ( ! self::check_php() ) {
			add_action( 'admin_notices', array( 'WPJMSF_Dependency_Checker', 'add_php_notice' ) );
		}

		if ( ! self::check_wp() ) {
			add_action( 'admin_notices', array( 'WPJMSF_Dependency_Checker', 'add_wp_notice' ) );
			add_filter( 'plugin_action_links_' . WPJM_SEARCH_FILTERING_BASENAME, array( 'WPJMSF_Dependency_Checker', 'wp_version_plugin_action_notice' ) );
		}
	...
	public static function add_php_notice() {
		...
		$message = sprintf( __( '<strong>WP Job Manager - Search and Filtering</strong> requires a minimum PHP version of %1$s, but you are running %2$s. Please update PHP to continue using this plugin.', 'wp-job-manager-search-and-filtering' ), self::MINIMUM_PHP_VERSION, phpversion() );
		...
		printf(
			...
			esc_html__( 'Learn more about updating PHP', 'wp-job-manager-search-and-filtering' ),
			/* translators: accessibility text */
			esc_html__( '(opens in a new tab)', 'wp-job-manager-search-and-filtering' )
		);
		echo '</p></div>';
	}
	...
	public static function add_wp_notice() {
		...
		echo '<p>' . wp_kses_post( __( '<strong>WP Job Manager - Search and Filtering</strong> requires a more recent version of WordPress.', 'wp-job-manager-search-and-filtering' ) . $update_action_link ) . '</p>';
		echo '</div>';
	}
	...
	public static function wp_version_plugin_action_notice( $actions ) {
		...
		if ( ! current_user_can( 'update_core' ) ) {
			$actions[] = '<strong style="color: red">' . esc_html__( 'WordPress Update Required', 'wp-job-manager-search-and-filtering' ) . '</strong>';
		} else {
			$actions[] = '<a href="' . esc_url( self_admin_url( 'update-core.php' ) ) . '" style="color: red">' . esc_html__( 'WordPress Update Required', 'wp-job-manager-search-and-filtering' ) . '</a>';
		}
