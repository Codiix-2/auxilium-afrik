...
class Promotion extends \Cariera_Packages\Package_Types\Promotion {
	 ...
	 * @var string
	 */
	protected $package_type = 'company_promotional_package';
	...
	public function __construct() {
		...
		// Dashboard Action.
		add_action( 'cariera_company_dashboard_column_actions', [ $this, 'dashboard_action' ], 10, 2 );
	...
	/**
	 * Add "Promote" action to the Companies Dashboard
	 *
	 * @since   0.9.9
	 * @version 0.9.18
	 *
	...
	public function dashboard_action( $listing, $actions ) {
		...
		echo '<a href="#cariera-packages-promotions" class="cariera-packages listing-dashboard-action-promote company-dashboard-action-promote popup-with-zoom-anim" data-listing-id="' . esc_attr( $listing->ID ) . '"><i class="las la-bolt"></i><span>' . esc_html__( 'Promote', 'cariera-packages' ) . '</span></a>';
	}

	/**
	 * Output 'Choose Promotion' modal in Companies Dashboard
	 *
	 * @since   0.9.9
	 * @version 0.9.11
	 */
	public function get_promotions_modal() {
		...
		$promotions        = get_option( 'cariera_packages_promotional_package' );
		$listing_dashboard = get_option( 'cariera_company_dashboard_page' );
		...
		get_job_manager_template(
			...
			[
				'title'         => esc_html__( 'Company Promotion', 'cariera-packages' ),
				'products'      => $products,
				'packages'      => $user_packages,
				'type'          => $this->package_type,
				'duration_meta' => '_company_promotion_duration',
			],
			'cariera-packages',
	...
	public static function create_user_promotion_package( $user_id, $product_id, $order_id, $listing_id = null ) {
		...
		$args = apply_filters(
			'cariera_packages_company_promotion_package_data',
			[
		...
		$duration = get_post_meta( $product_id, '_company_promotion_duration', true );
		...
		do_action( 'cariera_packages_company_promotion_package_meta', $promotion_id, $user_id, $product_id, $order_id );
