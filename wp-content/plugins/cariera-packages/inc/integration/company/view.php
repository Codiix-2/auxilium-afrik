...
namespace Cariera_Packages\Integration\Company;
...
class View {
	 ...
	 * @var string
	 */
	private $package_type = 'company_view_package';
	...
	public function __construct() {
		...
		add_action( 'cariera_packages_single_company', [ $this, 'form_output' ] );
		...
		// Single Company Listing Notice.
		add_action( 'cariera_single_company_listing_start', [ $this, 'view_company_notice' ] );
	}
	...
	public function single_template( $template, $type, $templates ) {
		...
		// Return if listing view package is required.
		...
		// Check if user can fix view the company listing.
		if ( $this->user_can_view( $post->ID ) ) {
			return $template;
		}

		$single_template = CARIERA_PACKAGES_PATH . 'templates/single-company.php';
	...
	/**
	 * Handle split view restriction for company listings.
	 *
	 * @since 0.9.7
	 *
	 * @param string  $output
	...
	public function handle_split_view_restriction( $output, $listing, $post_type ) {
		...
		$template = CARIERA_PACKAGES_PATH . 'templates/content-single-company.php';
	...
	public function user_can_view( $listing_id = false ) {
		// Here I need a check that will handle if the user has already checked the company.
		...
		if ( absint( $listing_author ) === $user_id ) {
			return true;
		}

		// Check if the user has a valid package for viewing this company.
		if ( $user_packages ) {
			foreach ( $user_packages as $package ) {
				if ( $this->can_view_listing( $user_id, $package, $listing_id ) ) {
					// Valid package found, the user can view the company.
					return true;
				}
			}
		}
	 ...
	 * @param int $listing_id       The ID of the listing to check in viewed companies.
	 */
	public function can_view_listing( $user_id, $user_package_id, $listing_id ) {
		...
		$viewed_companies = get_post_meta( $user_package_id, $prefix . 'viewed_companies', true );
		...
		// Check if viewed_companies is not empty and contains the listing ID.
		if ( empty( $viewed_companies ) ) {
			return false;
		}

		// Convert viewed_companies into an array of integers.
		$viewed_companies_array = array_map( 'intval', array_filter( explode( ',', $viewed_companies ), 'is_numeric' ) );

		// Check if the listing ID exists in the viewed_companies array.
		if ( ! in_array( $listing_id, $viewed_companies_array, true ) ) {
			return false;
		}
	...
	public function form_output() {
		...
		$packages      = \Cariera_Packages\Helpers::get_package_products( $this->package_type );
		$user_packages = \Cariera_Packages\Helpers::get_user_packages( $user_id, 'company_view_package', true );
		...
		<form method="post" id="<?php echo esc_attr( $this->package_type ); ?>_selection" class="cariera-package-selection">
			...
			<div class="cariera-packages company_packages">
				<?php
	...
	public function user_package_description( $package_id, $package_type ) {
		...
		$view_limit = get_post_meta( $package_id, 'cariera_packages_view_company_limit', true );
	...
	/**
	 * Return the number of used company views.
	 *
	 * @param int $package_id
	 */
	private function used_views_count( $package_id ) {
		...
		// Retrieve the viewed companies meta value.
		$company_ids = get_post_meta( $package_id, $prefix . 'viewed_companies', true );

		// Return the count of viewed companies, handling empty or invalid values gracefully.
		return ! empty( $company_ids ) ? count( array_filter( explode( ',', $company_ids ), 'is_numeric' ) ) : 0;
	}
	...
	public function package_description( $package_id, $package_type ) {
		...
		$view_limit = $package_id->_view_company_limit;
		...
		printf(
			esc_html(
				_n(
					'View %s single company listing.',
					'View %s single company listings.',
	...
	/**
	 * Creates a Company View Package for a User.
	 *
	 * @since   0.9.4
	 * @version 0.9.8
	 *
	...
	public static function create_user_view_package( $user_id, $product_id, $order_id, $listing_id = null ) {
		...
		$args = apply_filters(
			'cariera_packages_company_view_package_data',
			[
		...
		$view_limit = get_post_meta( $product_id, '_view_company_limit', true );

		// Update package-specific meta data.
		update_post_meta( $user_package_id, "{$prefix}view_company_limit", $view_limit );

		// Assign listing to the package if listing_id exists.
		if ( ! empty( $listing_id ) ) {
			update_post_meta( $user_package_id, "{$prefix}viewed_companies", $listing_id );
		} else {
			update_post_meta( $user_package_id, "{$prefix}viewed_companies", '' );
		}

		// Trigger a custom action hook for additional processing or integrations.
		do_action( 'cariera_packages_company_view_package_meta', $user_package_id, $user_id, $product_id, $order_id );
	 ...
	 * @param int $user_id          The ID of the user to validate against.
	 * @param int $user_package_id  The ID of the user package to validate.
	 * @param int $listing_id       The ID of the listing to check in viewed companiess.
	 */
	private function is_valid( $user_id, $user_package_id, $listing_id ) {
		...
		$user_id_meta     = get_post_meta( $user_package_id, $prefix . 'user_id', true );
		$view_limit       = get_post_meta( $user_package_id, $prefix . 'view_company_limit', true );
		$viewed_companies = get_post_meta( $user_package_id, $prefix . 'viewed_companies', true );
		...
		if ( count( $viewed_companies_array ) < absint( $view_limit ) ) {
			$is_valid = true;
		}
	...
	private function add_viewed_listing( $user_id, $user_package_id, $listing_id ) {
		...
		// Fetch the current list of viewed companies.
		$viewed_companies = get_post_meta( $user_package_id, $prefix . 'viewed_companies', true );

		// Process the viewed companies list.
		if ( ! empty( $viewed_companies ) ) {
			$viewed_companies_array = array_map( 'trim', explode( ',', $viewed_companies ) );

			// Add the new listing ID if it's not already in the list
			if ( ! in_array( $listing_id, $viewed_companies_array, true ) ) {
				$viewed_companies_array[] = $listing_id;
			}
		} else {
			$viewed_companies_array = [ $listing_id ];
		}
		...
		$updated = update_post_meta( $user_package_id, $prefix . 'viewed_companies', implode( ',', $viewed_companies_array ) );
	...
	public function user_packages_count( $packages ) {
		$user_id       = get_current_user_id();
		$user_packages = \Cariera_Packages\Helpers::get_user_packages( $user_id, 'company_view_package' );
	...
	public function user_packages_content() {
		$user_id       = get_current_user_id();
		$view_packages = \Cariera_Packages\Helpers::get_user_packages( $user_id, 'company_view_package' );
		...
		foreach ( $view_packages as $package_id ) {
			...
			<tr>
				...
				<td class="package-title">
					...
					<p><?php wp_kses_post( printf( __( 'You have <span>%s</span> company views left.', 'cariera-packages' ), $view_limit ? absint( $view_limit - $used_views ) : esc_html__( 'Unlimited', 'cariera-packages' ) ) ); ?></p>
				</td>

				<td class="package-type"><?php esc_html_e( 'Company View', 'cariera-packages' ); ?></td>
	...
	public function active_packages_content( $displayed_packages, $max_packages ) {
		$user_id       = get_current_user_id();
		$view_packages = \Cariera_Packages\Helpers::get_user_packages( $user_id, 'company_view_package', true );
		...
		foreach ( $view_packages as $package_id ) {
			...
			<li class="package">
				...
				<div class="content">
					...
					<p><?php printf( esc_html__( 'You can view %s more company listings.', 'cariera-packages' ), $view_limit ? absint( $view_limit - $used_views ) : esc_html__( 'Unlimited', 'cariera-packages' ) ); ?></p>
				</div>
			</li>
			<?php
		}
	...
	/**
	 * Add notice when company can be viewed.
	 *
	 * @since 0.9.4
	 */
	public function view_company_notice() {
		if ( ! isset( $_GET['notice'] ) || 'listing_viewed' !== $_GET['notice'] ) {
			return;
		}

		echo '<div class="job-manager-message success">' . esc_html__( 'You can now view this company listing!', 'cariera-packages' ) . '</div>';
	}
}
