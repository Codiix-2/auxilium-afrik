...
class View {
	 ...
	 * @var string
	 */
	private $package_type = 'job_view_package';
	...
	public function __construct() {
		...
		add_action( 'cariera_packages_single_job_listing', [ $this, 'form_output' ] );
		...
		// Single Job Listing Notice.
		add_action( 'single_job_listing_start', [ $this, 'view_job_notice' ] );
	}
	...
	public function single_template( $template, $type, $templates ) {
		...
		// Return if job view package is required.
		...
		// Check if user can fix view the job listing.
		if ( $this->user_can_view( $post->ID ) ) {
			return $template;
		}

		$single_template = CARIERA_PACKAGES_PATH . 'templates/single-job_listing.php';
	...
	/**
	 * Handle split view restriction for job listings.
	 *
	 * @since 0.9.7
	 *
	 * @param string  $output
	...
	public function handle_split_view_restriction( $output, $listing, $post_type ) {
		...
		$template = CARIERA_PACKAGES_PATH . 'templates/content-single-job_listing.php';
	...
	public function user_can_view( $listing_id = false ) {
		// Here I need a check that will handle if the user has already checked the job.
		...
		// Check if the user has a valid package for viewing this job.
		if ( $user_packages ) {
			foreach ( $user_packages as $package ) {
				if ( $this->can_view_listing( $user_id, $package, $listing_id ) ) {
					// Valid package found, the user can view the job.
					return true;
				}
			}
		}
	 ...
	 * @param int $listing_id       The ID of the listing to check in viewed jobs.
	 */
	public function can_view_listing( $user_id, $user_package_id, $listing_id ) {
		...
		$viewed_jobs  = get_post_meta( $user_package_id, $prefix . 'viewed_jobs', true );
		...
		// Check if viewed_jobs is not empty and contains the listing ID.
		if ( empty( $viewed_jobs ) ) {
			return false;
		}

		// Convert viewed_jobs into an array of integers.
		$viewed_jobs_array = array_map( 'intval', array_filter( explode( ',', $viewed_jobs ), 'is_numeric' ) );

		// Check if the listing ID exists in the viewed_jobs array.
		if ( ! in_array( $listing_id, $viewed_jobs_array, true ) ) {
			return false;
		}
	...
	public function form_output() {
		...
		$packages      = \Cariera_Packages\Helpers::get_package_products( $this->package_type );
		$user_packages = \Cariera_Packages\Helpers::get_user_packages( $user_id, 'job_view_package', true );
		...
		<form method="post" id="<?php echo esc_attr( $this->package_type ); ?>_selection" class="cariera-package-selection">
			...
			<div class="cariera-packages job_listing_packages">
				<?php
	...
	public function user_package_description( $package_id, $package_type ) {
		...
		$view_limit = get_post_meta( $package_id, 'cariera_packages_view_job_limit', true );
	...
	/**
	 * Return the number of used job views.
	 *
	 * @param int $package_id
	 */
	private function used_views_count( $package_id ) {
		...
		// Retrieve the viewed jobs meta value.
		$job_ids = get_post_meta( $package_id, $prefix . 'viewed_jobs', true );

		// Return the count of viewed jobs, handling empty or invalid values gracefully.
		return ! empty( $job_ids ) ? count( array_filter( explode( ',', $job_ids ), 'is_numeric' ) ) : 0;
	}
	...
	public function package_description( $package_id, $package_type ) {
		...
		$view_limit = $package_id->_view_job_limit;
		...
		printf(
			esc_html(
				_n(
					'View %s single job listing.',
					'View %s single job listings.',
	...
	/**
	 * Creates a Job View Package for a User.
	 *
	 * @since   0.9.0
	 * @version 0.9.9
	 *
	...
	public static function create_user_view_package( $user_id, $product_id, $order_id, $listing_id = null ) {
		...
		$args = apply_filters(
			'cariera_packages_job_view_package_data',
			[
		...
		$view_limit = get_post_meta( $product_id, '_view_job_limit', true );

		// Update package-specific meta data.
		update_post_meta( $user_package_id, "{$prefix}view_job_limit", $view_limit );

		// Assign listing to the package if listing_id exists.
		if ( ! empty( $listing_id ) ) {
			update_post_meta( $user_package_id, "{$prefix}viewed_jobs", $listing_id );
		} else {
			update_post_meta( $user_package_id, "{$prefix}viewed_jobs", '' );
		}

		// Trigger a custom action hook for additional processing or integrations.
		do_action( 'cariera_packages_job_view_package_meta', $user_package_id, $user_id, $product_id, $order_id );
	...
	private function is_valid( $user_id, $user_package_id, $listing_id ) {
		...
		$user_id_meta = get_post_meta( $user_package_id, $prefix . 'user_id', true );
		$view_limit   = get_post_meta( $user_package_id, $prefix . 'view_job_limit', true );
		$viewed_jobs  = get_post_meta( $user_package_id, $prefix . 'viewed_jobs', true );
		...
		if ( count( $viewed_jobs_array ) < absint( $view_limit ) ) {
			$is_valid = true;
		}
	...
	private function add_viewed_listing( $user_id, $user_package_id, $listing_id ) {
		...
		// Fetch the current list of viewed jobs.
		$viewed_jobs = get_post_meta( $user_package_id, $prefix . 'viewed_jobs', true );

		// Process the viewed jobs list.
		if ( ! empty( $viewed_jobs ) ) {
			$viewed_jobs_array = array_map( 'trim', explode( ',', $viewed_jobs ) );

			// Add the new listing ID if it's not already in the list.
			if ( ! in_array( $listing_id, $viewed_jobs_array, true ) ) {
				$viewed_jobs_array[] = $listing_id;
			}
		} else {
			$viewed_jobs_array = [ $listing_id ];
		}
		...
		$updated = update_post_meta( $user_package_id, $prefix . 'viewed_jobs', implode( ',', $viewed_jobs_array ) );
	...
	public function user_packages_count( $packages ) {
		$user_id       = get_current_user_id();
		$user_packages = \Cariera_Packages\Helpers::get_user_packages( $user_id, 'job_view_package' );
	...
	public function user_packages_content() {
		$user_id       = get_current_user_id();
		$view_packages = \Cariera_Packages\Helpers::get_user_packages( $user_id, 'job_view_package' );
		...
		foreach ( $view_packages as $package_id ) {
			...
			<tr>
				...
				<td class="package-title">
					...
					<p><?php wp_kses_post( printf( __( 'You have <span>%s</span> job views left.', 'cariera-packages' ), $view_limit ? absint( $view_limit - $used_views ) : esc_html__( 'Unlimited', 'cariera-packages' ) ) ); ?></p>
				</td>

				<td class="package-type"><?php esc_html_e( 'Job View', 'cariera-packages' ); ?></td>
	...
	public function active_packages_content( $displayed_packages, $max_packages ) {
		$user_id       = get_current_user_id();
		$view_packages = \Cariera_Packages\Helpers::get_user_packages( $user_id, 'job_view_package', true );
		...
		foreach ( $view_packages as $package_id ) {
			...
			<li class="package">
				...
				<div class="content">
					...
					<p><?php printf( esc_html__( 'You can view %s more job listings.', 'cariera-packages' ), $view_limit ? absint( $view_limit - $used_views ) : esc_html__( 'Unlimited', 'cariera-packages' ) ); ?></p>
				</div>
			</li>
			<?php
		}
	...
	/**
	 * Add notice when job can be viewed.
	 *
	 * @since 0.9.4
	 */
	public function view_job_notice() {
		if ( ! isset( $_GET['notice'] ) || 'listing_viewed' !== $_GET['notice'] ) {
			return;
		}

		echo '<div class="job-manager-message success">' . esc_html__( 'You can now view this job listing!', 'cariera-packages' ) . '</div>';
	}
}
