...
class TRP_IN_SP_Slug_Manager {
    ...
    public function include_slug_for_machine_translation( $translateable_information = array(), $html = false, $no_translate_attribute = '', $TRP_LANGUAGE = null, $language_code = '', $translation_render = false ) {
        ...
        global $TRP_LANGUAGE;
        if ( empty( $language_code ) && ! empty( $TRP_LANGUAGE ) ) {
...
//            }
            ...
            // post slugs
            if ( isset( $post->ID ) && !empty( $post->ID ) && isset( $post->post_name ) && !empty( $post->post_name ) && !is_home() && !is_front_page() && !is_archive() && !is_search() && ( !isset( $post->post_type ) || $post->post_type !== 'attachment' ) ) {

                $this->add_post_slug_for_translation( $post, $translateable_information, $skip_automatic_translation_for_slugs, $language_code );

                // traverse parent pages for hierarchical post types (e.g. /page1/page2/page3/)
                if ( !empty( $post->post_parent ) ) {
                    $parent_id = $post->post_parent;
                    $depth     = 0;
                    while ( $parent_id && $depth < 20 ) {
                        $parent_post = get_post( $parent_id );
                        if ( !$parent_post ) {
                            break;
                        }
                        $this->add_post_slug_for_translation( $parent_post, $translateable_information, $skip_automatic_translation_for_slugs, $language_code );
            ...
            // archives
            if ( is_archive() ) {
                // skip known non-taxonomy query vars instead of blocking when post_type is set
                $trp_skip_query_vars = array( 'post_type', 'paged', 'posts_per_page' );
                ...
                foreach ( $wp_query->query as $taxonomy => $term_slug ) {
                    if ( in_array( $taxonomy, $trp_skip_query_vars ) ) {
                        continue;
                    }
                    ...
                    if ( in_array( $actual_taxonomy, $trp_all_taxonomies ) ) {
                        ...
                        // handle hierarchical term slugs (e.g. 'clothing/accessories')
                        if ( is_string( $term_slug ) && strpos( $term_slug, '/' ) !== false ) {
                            $slug_segments = explode( '/', $term_slug );
                            foreach ( $slug_segments as $single_slug ) {
                                if ( empty( $single_slug ) ) {
                                    continue;
                                }
                                $segment_term_object = get_term_by( 'slug', $single_slug, $actual_taxonomy );
                                if ( $segment_term_object ) {
                                    $this->add_term_slug_for_translation( $single_slug, $segment_term_object, $translateable_information, $skip_automatic_translation_for_slugs, $language_code, $woo_english_slugs );
                                }
                            }
                        } else {
                            ...
                            if ( isset( $term_object->term_id ) ) {
                                $this->add_term_slug_for_translation( $term_slug, $term_object, $translateable_information, $skip_automatic_translation_for_slugs, $language_code, $woo_english_slugs );
                            }
                        }
    ...
    /**
     * Helper to add a term slug to $translateable_information for machine translation.
     *
     * @param string $term_slug           The term slug to add.
     * @param object $term_object          The term object (must have term_id).
     * @param array  $translateable_information  Reference to the translateable information array.
     * @param bool   $skip_automatic_translation_for_slugs  Whether AT is off.
     * @param string $language_code        Target language code.
     * @param array  $woo_english_slugs    WooCommerce base slugs to exclude.
     *
     * @return bool True if the slug was added, false otherwise.
     */
    private function add_term_slug_for_translation( $term_slug, $term_object, &$translateable_information, $skip_automatic_translation_for_slugs, $language_code, $woo_english_slugs ) {
        if ( !isset( $term_object->term_id ) ) {
            return false;
        }
    ...
    /**
     * Helper to add a post slug to $translateable_information for machine translation.
     *
     ...
     * @return bool True if the slug was added, false otherwise.
     */
    private function add_post_slug_for_translation( $post_object, &$translateable_information, $skip_automatic_translation_for_slugs, $language_code ) {
        if ( !isset( $post_object->post_name ) || empty( $post_object->post_name ) ) {
            return false;
        }

        if ( ( $skip_automatic_translation_for_slugs &&
                !$this->verify_if_slug_original_exists_already_for_when_slug_machine_translation_is_off( $post_object->post_name ) ) ||
            ( !$skip_automatic_translation_for_slugs && !$this->verify_if_the_slug_translation_already_exists_for_machine_translation( $post_object->post_name, $language_code ) ) ) {

            $is_public = true;
            $is_it_a_standard_post = true;

            $verify_if_the_post_type_is_public = isset( $post_object->post_type ) ? get_post_type_object( $post_object->post_type ) : false;
            ...
            } elseif ( !$verify_if_the_post_type_is_public->publicly_queryable && empty( $verify_if_the_post_type_is_public->public ) ) {
                $is_public = false;
            }

            if ( $is_public && $is_it_a_standard_post ) {
                $post_trimmed = trp_full_trim($post_object->post_name, array('numerals' => 'no'));
                if ( !empty( $post_trimmed ) ) {
                    ...
                    $translateable_information['nodes'][] = array('type' => 'post', 'post_id' => $post_object->ID, 'original_slug_name' => $post_object->post_name, 'skip_automatic_translation' => $skip_automatic_translation_for_slugs);
                    return true;
                }
            }
        }
    ...
    protected function filter_woocommerce_base_slugs_for_request_uri( $slug_pairs ) {
        ...
        foreach ( $wc_base_slugs as $english_slug ) {
            ...
            // Segment-based slug replacement cannot safely inject composite permalink paths.
            if ( empty( $actual_permalink_slug ) || strpos( $actual_permalink_slug, '/' ) !== false ) {
                continue;
            }
    ...
    public function add_woocommerce_gettext_slug_pairs( $slug_pairs, $translatable_slugs, $target_language ) {
        // Request-local cache for "does this custom WC base already have an explicit slug translation?"
        static $wc_custom_base_has_translation_cache = array();
        ...
        foreach ( $wc_base_slugs as $english_slug ) {
            ...
            // If TranslatePress already has an explicit slug-table translation for that custom base,
            // do not add a WooCommerce gettext fallback pair that would override it.
            if ( $has_custom_permalink ) {
                $cache_key = $target_language . '|' . strtolower( $actual_permalink_slug );

                if ( ! array_key_exists( $cache_key, $wc_custom_base_has_translation_cache ) ) {
                    $wc_custom_base_has_translation_cache[ $cache_key ] = ! empty(
                        $this->slug_query->get_translated_slugs_from_original( array( $actual_permalink_slug ), $target_language )
                    );
                }

                $has_custom_base_translation = $wc_custom_base_has_translation_cache[ $cache_key ];

                // A real saved translation should win over this fallback layer.
                if ( $has_custom_base_translation ) {
                    continue;
                }
