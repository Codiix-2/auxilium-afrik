 ...
 *
 * This template can be overridden by copying it to yourtheme/cariera-addons/resumes/emails/admin-new-resume.php.
 *
...
echo '<p>';
// translators: %1$s is the URL for the site; %2$s is the name of the site; %3$s is the singular resume label.
echo wp_kses_post(
	sprintf(
		__( 'A new %3$s has been submitted to <a href="%1$s">%2$s</a>.', 'cariera-addons' ),
		home_url(),
		get_bloginfo( 'name' ),
...
switch ( $resume->post_status ) {
	...
	case 'pending':
		...
		echo ' ' . wp_kses_post( sprintf( __( 'It is awaiting approval by an administrator in <a href="%s">WordPress admin</a>.', 'cariera-addons' ), esc_url( admin_url( 'edit.php?post_type=resume' ) ) ) );
		break;
}
