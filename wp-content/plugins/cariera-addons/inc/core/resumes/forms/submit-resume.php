...
class Submit_Resume extends \WP_Job_Manager_Form {
	...
	public function __construct() {
		...
		$this->steps = (array) apply_filters(
			...
			[
				'submit'  => [
					'name'     => esc_html__( 'Submit Details', 'cariera-addons' ),
					'view'     => [ $this, 'submit' ],
					'handler'  => [ $this, 'submit_handler' ],
					'priority' => 10,
				],
				'preview' => [
					'name'     => esc_html__( 'Preview', 'cariera-addons' ),
					'view'     => [ $this, 'preview' ],
					'handler'  => [ $this, 'preview_handler' ],
					'priority' => 20,
				],
				'done'    => [
					'name'     => esc_html__( 'Done', 'cariera-addons' ),
					'view'     => [ $this, 'done' ],
					'handler'  => '',
					'priority' => 30,
				],
		...
		if (
			empty( $this->job_id )
			|| \WP_Job_Manager_Post_Types::PT_LISTING !== get_post_type( $this->job_id )
		...
		switch ( $field['type'] ) {
			...
			case 'links':
				get_job_manager_template(
					'resumes/form-fields/repeated-field.php',
					[
						'key'   => $key,
						'field' => $field,
						'class' => $this,
	...
	public function init_fields() {
		...
		if ( $max_skills ) {
			...
			$max_skills_notice = ' ' . sprintf( __( 'Maximum of %d.', 'cariera-addons' ), $max_skills );
		}
		...
		$this->fields = apply_filters(
			...
			[
				'resume_fields' => [
					'candidate_name'       => [
						'label'         => esc_html__( 'Your name', 'cariera-addons' ),
						'type'          => 'text',
						'required'      => true,
						'placeholder'   => esc_html__( 'Your full name', 'cariera-addons' ),
						'priority'      => 1,
						'personal_data' => true,
					],
					'candidate_email'      => [
						'label'         => esc_html__( 'Your email', 'cariera-addons' ),
						'type'          => 'text',
						'required'      => true,
					...
					'candidate_title'      => [
						...
						'type'          => 'text',
						'required'      => true,
						'placeholder'   => esc_html__( 'e.g. "Web Developer"', 'cariera-addons' ),
						'priority'      => 3,
						'personal_data' => true,
					],
					'candidate_location'   => [
					...
					'candidate_video'      => [
						...
						'required'      => false,
						'priority'      => 6,
						'placeholder'   => esc_html__( 'A link to a video about yourself', 'cariera-addons' ),
						'personal_data' => true,
					],
					'resume_category'      => [
						// translators: Placeholder is the singular resume label, e.g. "Resume".
						'label'         => sprintf( esc_html__( '%s category', 'cariera-addons' ), $singular_label ),
					...
					'resume_content'       => [
						// translators: Placeholder is the singular resume label, e.g. "Resume".
						'label'         => sprintf( esc_html__( '%s Content', 'cariera-addons' ), $singular_label ),
						'type'          => 'wp-editor',
						'required'      => true,
						'placeholder'   => '',
						'priority'      => 8,
					...
					'resume_skills'        => [
						...
						'type'          => 'text',
						'required'      => false,
						'placeholder'   => esc_html__( 'Comma separate a list of relevant skills', 'cariera-addons' ) . $max_skills_notice,
						'priority'      => 9,
						'personal_data' => true,
					],
					'links'                => [
						'label'         => esc_html__( 'URL(s)', 'cariera-addons' ),
						'add_row'       => esc_html__( 'Add URL', 'cariera-addons' ),
						'type'          => 'links', // Repeated field.
						'required'      => false,
						'placeholder'   => '',
						'description'   => esc_html__( 'Optionally provide links to any of your websites or social network profiles.', 'cariera-addons' ),
						'priority'      => 10,
						'fields'        => [
							'name' => [
					...
					'candidate_education'  => [
						...
						'add_row'       => esc_html__( 'Add Education', 'cariera-addons' ),
					...
					'candidate_experience' => [
						...
						'add_row'       => esc_html__( 'Add Experience', 'cariera-addons' ),
					...
					'resume_file'          => [
						// translators: Placeholder is the singular resume label, e.g. "Resume".
						'label'         => sprintf( esc_html__( '%s file', 'cariera-addons' ), $singular_label ),
						'type'          => 'file',
						'required'      => false,
						'ajax'          => true,
						// translators: 1: Resume label, 2: Maximum upload file size.
						'description'   => sprintf( esc_html__( 'Optionally upload your %1$s for employers to view. Max. file size: %2$s.', 'cariera-addons' ), $singular_label, size_format( wp_max_upload_size() ) ),
						'priority'      => 13,
						'placeholder'   => '',
						'personal_data' => true,
					],
		...
		if ( ! get_option( 'resume_manager_enable_categories' ) || 0 === wp_count_terms( \Cariera_Addons\Core\Resumes\Post_Types::TAX_CATEGORY ) ) {
	...
	/**
	 * Reset the `fields` variable so it gets reinitialized. This should only be used for testing!
	 *
	 * @since 0.9.5
	 */
		...
		foreach ( $this->fields as $group_key => $fields ) {
			foreach ( $fields as $key => $field ) {
				if ( $field['required'] && $this->is_empty( $values[ $group_key ][ $key ] ) ) {
					...
					return new \WP_Error( 'validation-error', sprintf( __( '%s is a required field', 'cariera-addons' ), $field['label'] ) );
				}
					...
					if ( is_array( $values[ $group_key ][ $key ] ) ) {
						foreach ( $values[ $group_key ][ $key ] as $term ) {
							if ( ! term_exists( $term, $field['taxonomy'] ) ) {
								...
								return new \WP_Error( 'validation-error', sprintf( __( '%s is invalid', 'cariera-addons' ), $field['label'] ) );
							}
						}
				...
				if ( 'candidate_email' === $key ) {
					if ( ! empty( $values[ $group_key ][ $key ] ) && ! is_email( $values[ $group_key ][ $key ] ) ) {
						throw new \Exception( esc_html__( 'Please enter a valid email address', 'cariera-addons' ) );
					}
				}
				...
				if ( 'resume_skills' === $key ) {
					...
					if ( $max && count( $raw_skills ) > $max ) {
						...
						return new \WP_Error( 'validation-error', sprintf( __( 'Please enter no more than %d skills.', 'cariera-addons' ), $max ) );
					}
				}
				...
				if ( 'file' === $field['type'] ) {
					...
					if ( ! empty( $check_value ) ) {
						foreach ( $check_value as $file_url ) {
							...
							if ( empty( $file_url ) ) {
								throw new \Exception( esc_html__( 'Invalid attachment provided.', 'cariera-addons' ) );
							}
						}
					}
				}
	...
	public function submit() {
				...
				foreach ( $fields as $key => $field ) {
					switch ( $key ) {
						...
						case 'resume_category':
							$this->fields[ $group_key ][ $key ]['value'] = wp_get_object_terms( $resume->ID, \Cariera_Addons\Core\Resumes\Post_Types::TAX_CATEGORY, [ 'fields' => 'ids' ] );
							break;
						default:
		...
		get_job_manager_template(
			...
			[
				...
				'step'               => $this->get_step(),
				'submit_button_text' => apply_filters( 'submit_resume_form_submit_button_text', __( 'Preview &rarr;', 'cariera-addons' ) ),
			],
			'cariera-addons',
			CARIERA_ADDONS_PATH . '/templates/'
		);
	...
	public function submit_handler() {
		try {
				...
				if ( resume_manager_enable_registration() ) {
					if ( resume_manager_user_requires_account() ) {
						if ( ! resume_manager_generate_username_from_email() && empty( $input_create_account_username ) ) {
							throw new \Exception( __( 'Please enter a username.', 'cariera-addons' ) );
						}
						if ( ! resume_manager_use_standard_password_setup_email() ) {
							if ( empty( $input_create_account_password ) ) {
								throw new \Exception( __( 'Please enter a password.', 'cariera-addons' ) );
							}
						}
						if ( empty( $input_create_account_email ) ) {
							throw new \Exception( __( 'Please enter your email address.', 'cariera-addons' ) );
						}
					}
					...
					if ( ! resume_manager_use_standard_password_setup_email() && ! empty( $input_create_account_password ) ) {
						if ( empty( $input_create_account_password_verify ) || $input_create_account_password_verify !== $input_create_account_password ) {
							throw new \Exception( __( 'Passwords must match.', 'cariera-addons' ) );
						}
						if ( ! wpjm_validate_new_password( $input_create_account_password ) ) {
							...
							if ( $password_hint ) {
								...
								throw new \Exception( sprintf( __( 'Invalid Password: %s', 'cariera-addons' ), $password_hint ) );
							} else {
								throw new \Exception( __( 'Password is not valid.', 'cariera-addons' ) );
							}
						}
					}
			...
			if ( resume_manager_user_requires_account() && ! is_user_logged_in() ) {
				throw new \Exception( __( 'You must be signed in to post your resume.', 'cariera-addons' ) );
			}
	...
	protected function update_resume_data( $values ) {
		...
		if ( get_option( 'resume_manager_enable_skills' ) && isset( $values['resume_fields']['resume_skills'] ) ) {
			...
			if ( is_string( $raw_tags ) ) {
				...
				if ( ! empty( $raw_tags ) ) {
					foreach ( $raw_tags as $tag ) {
						$term = get_term_by( 'name', $tag, \Cariera_Addons\Core\Resumes\Post_Types::TAX_SKILL );
						if ( $term ) {
							$tags[] = $term->term_id;
						} else {
							$term = wp_insert_term( $tag, \Cariera_Addons\Core\Resumes\Post_Types::TAX_SKILL );
			...
			wp_set_object_terms( $this->resume_id, $tags, \Cariera_Addons\Core\Resumes\Post_Types::TAX_SKILL, false );
		}
	...
	public function done() {
		...
		if ( $this->job_id ) {
			get_job_manager_template(
				'resumes/resume-submitted-application-form.php',
				[
					'resume' => get_post( $this->resume_id ),
					'job_id' => $this->job_id,
				],
	...
	private function check_valid_resume() {
		if (
			...
			&& \Cariera_Addons\Core\Resumes\Post_Types::CPT_RESUME === get_post_type( $this->resume_id )
		) {
			return;
		}

		wp_die(
			esc_html__( 'Invalid resume', 'cariera-addons' ),
			'',
			[
				'response'  => 400,
				'back_link' => true,
