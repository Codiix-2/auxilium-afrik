...
class Apply extends \WP_Job_Manager_Form {

	use \Cariera_Addons\Src\Traits\Singleton;

	/**
	 * The fields to display on the application form.
	 *
	 * @var array
	 */
	protected $fields = [];
	...
	/**
	 * The error message to display on the application form.
	 *
	 * @var string
	 */
	private $error = '';
	...
	/**
	 * The secret directory for the application form.
	 *
	 * @var string
	 */
	...
	/**
	 * Possibly enables recaptcha and enqueues scripts for the application form.
	 *
	 * @since   0.9.3
	 * @version 0.9.7
	 */
	public function form_setup() {
		if ( ! is_singular( \WP_Job_Manager_Post_Types::PT_LISTING ) ) {
			return;
		}
		...
		if ( class_exists( 'WP_Job_Manager\WP_Job_Manager_Recaptcha' ) ) {
			\WP_Job_Manager\WP_Job_Manager_Recaptcha::instance()->maybe_enable_recaptcha(
				'job_application_enable_recaptcha_application_submission',
				[ 'job_application_form_fields_end' ],
				[ 'application_form_validate_fields' ]
			);
		} else {
			// This path will be followed for WPJM versions pre-3.2.0.
		...
		// Enqueue reCaptcha scripts for the application form.
	...
	private function sanitize_text_field_with_linebreaks( $input ) {
		...
		return str_replace( '[nl]', "\n", sanitize_text_field( str_replace( "\n", '[nl]', wp_strip_all_tags( stripslashes( $input ) ) ) ) );
	}
	...
	public function init_fields() {
		 ...
		 */
		$application_form_id = apply_filters( 'job_application_form_id', get_post_meta( $post->ID, \Cariera_Addons\Core\Applications\Application_Form::FORM_POST_META_KEY, true ), $post->ID );
		...
		foreach ( $this->fields as $key => $field ) {
			...
			if ( 'select' === $field['type'] && ! $this->fields[ $key ]['required'] ) {
				...
				$this->fields[ $key ]['options'] = array_merge( [ 0 => esc_html__( 'Choose an option', 'cariera-addons' ) ], $options );
			}
			if ( 'resumes' === $field['type'] ) {
				if ( function_exists( 'get_resume_share_link' ) && is_user_logged_in() ) {
					...
					foreach ( $resume_posts as $resume ) {
						...
						} elseif ( function_exists( 'get_the_candidate_title' ) ) {
							$resume_title = get_the_candidate_title( $resume );
				...
				if ( $this->fields[ $key ]['required'] && 1 === count( $resumes ) ) {
					...
					if ( ! $this->fields[ $key ]['required'] ) {
						$resumes = [ 0 => esc_html__( 'Choose an online resume...', 'cariera-addons' ) ] + $resumes;
					}
			...
			$this->fields[ $key ]['value'] = isset( $_POST[ $key ] ) ? $this->sanitize_text_field_with_linebreaks( $_POST[ $key ] ) : $this->fields[ $key ]['value']; // phpcs:ignore
		}
		...
		switch ( $field['type'] ) {
			case 'single-resume':
				get_job_manager_template(
					'applications/form-fields/single-resume-field.php',
					[
						'key'   => $key,
						'field' => $field,
					],
	...
	public function disable_application_form( $template, $template_name ) {
		...
		if ( 'job-application.php' === $template_name && get_option( 'job_application_prevent_multiple_applications' ) && user_has_applied_for_job( get_current_user_id(), $post->ID ) ) {
			return locate_job_manager_template( 'applications/application-form-applied.php', 'cariera-addons', CARIERA_ADDONS_PATH . '/templates/' );
		}
		return $template;
	}
	...
	public function application_form() {
		if ( get_option( 'job_application_form_require_login', 0 ) && ! is_user_logged_in() ) {
			get_job_manager_template( 'applications/application-form-login.php', [], 'cariera-addons', CARIERA_ADDONS_PATH . '/templates/' );
		} else {
			$this->init_fields();

			$form_id = get_post_meta( get_the_ID(), \Cariera_Addons\Core\Applications\Application_Form::FORM_POST_META_KEY, true );
			wp_enqueue_script( 'cariera-addons-applications' );

			get_job_manager_template(
				'applications/application-form.php',
				[
	 ...
	 * @version 1.0.2
	 *
	 * @throws \Exception When the application cannot be processed.
	 */
	public function application_form_handler() {
		...
		// phpcs:ignore
		if ( ! empty( $_POST['cariera_addons_send_application'] ) ) {
			try {
				...
				if ( empty( $job_id ) || ! $job || \WP_Job_Manager_Post_Types::PT_LISTING !== $job->post_type ) {
					throw new \Exception( esc_html__( 'Invalid job', 'cariera-addons' ) );
				}

				if ( 'publish' !== $job->post_status ) {
					throw new \Exception( esc_html__( 'That job is not available', 'cariera-addons' ) );
				}

				if ( get_option( 'job_application_prevent_multiple_applications' ) ) {
					$user_id = get_current_user_id();
					$email   = isset( $_POST['candidate_email'] ) ? sanitize_email( $_POST['candidate_email'] ) : ''; // phpcs:ignore

					// Check logged-in users by user ID.
					if ( $user_id && user_has_applied_for_job( $user_id, $job_id ) ) {
						throw new \Exception( esc_html__( 'You have already applied for this job', 'cariera-addons' ) );
					}

					// Check non-logged-in users by email.
				...
				foreach ( $fields as $key => $field ) {
					...
					switch ( $field['type'] ) {
						case 'file':
							...
							break;
						default:
							$values[ $key ] = isset( $_POST[ $key ] ) ? $this->sanitize_text_field_with_linebreaks( $_POST[ $key ] ) : ''; // phpcs:ignore
							break;
					}
					...
					if ( $field['required'] && empty( $values[ $key ] ) ) {
						...
						throw new \Exception( sprintf( esc_html__( '"%s" is a required field', 'cariera-addons' ), $field['label'] ) );
					}
							...
							switch ( $rule ) {
								...
								case 'from_email':
									if ( ! is_email( $values[ $key ] ) ) {
										throw new \Exception( $field['label'] . ': ' . esc_html__( 'Please provide a valid email address', 'cariera-addons' ) );
									}
									break;
								case 'numeric':
									if ( ! is_numeric( $values[ $key ] ) ) {
										throw new \Exception( $field['label'] . ': ' . esc_html__( 'Please enter a number', 'cariera-addons' ) );
									}
									break;
							}
						}
				...
				foreach ( $fields as $key => $field ) {
					...
					if ( in_array( 'message', $field['rules'], true ) ) {
						$application_message[] = $values[ $key ];
					}

					if ( in_array( 'resume_id', $field['rules'], true ) ) {
					...
					if ( 'file' === $field['type'] ) {
						if ( ! empty( $values[ $key ] ) ) {
							...
							foreach ( $values[ $key ] as $attachment ) {
								if ( ! is_wp_error( $attachment ) ) {
									if ( in_array( 'attachment', $field['rules'], true ) ) {
						...
						$meta[ $field['label'] ] = $values[ $key ] ? __( 'Yes', 'cariera-addons' ) : __( 'No', 'cariera-addons' );
				...
				// Create application.
				$application_id = create_job_application( $job_id, $from_name, $from_email, $application_message, $meta );
				if ( ! $application_id ) {
					throw new \Exception( esc_html__( 'Failed to create the job application.', 'cariera-addons' ) );
				}
				...
				if ( $candidate_email_content ) {
					...
					$GLOBALS['shortcode_tags'] = $existing_shortcode_tags; // phpcs:ignore
					$headers                   = [];
				...
				do_action_deprecated( 'new_job_application', [ $application_id, $job_id ], '2.5.3', 'job_manager_applications_new_job_application' );
	 ...
	 * @param array  $field
	 * @throws \Exception When the file cannot be uploaded.
	 */
	public function upload_file( $field_key, $field ) {
		...
		if ( isset( $_FILES[ $field_key ] ) && ! empty( $_FILES[ $field_key ] ) && ! empty( $_FILES[ $field_key ]['name'] ) ) {
			...
			$files           = [];
			$files_to_upload = job_manager_prepare_uploaded_files( $_FILES[ $field_key ] ); // phpcs:ignore
	...
	public function application_form_success() {
		get_job_manager_template( 'applications/application-submitted.php', [], 'cariera-addons', CARIERA_ADDONS_PATH . '/templates/' );
	}
	...
	public function application_form_errors() {
		if ( $this->error ) {
			echo '<p class="job-manager-message error">' . esc_html( $this->error ) . '</p>';
		}
	}
}
