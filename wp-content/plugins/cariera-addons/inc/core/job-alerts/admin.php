...
class Admin {
	...
	public function add_job_alert_columns( $columns ) {
		$new_columns = [
			'cb'         => $columns['cb'],
			'title'      => $columns['title'],
			'terms'      => esc_html__( 'Search Terms', 'cariera-addons' ),
			'alert_user' => esc_html__( 'User', 'cariera-addons' ),
		];
	...
	public function modify_job_alert_post_state( $post_states, $post ) {
		...
		if ( Post_Types::CPT_ALERT === $post->post_type && 'draft' === $post->post_status ) {
			$post_states = [ esc_html__( 'Disabled', 'cariera-addons' ) ];
		}
	...
	public function hide_taxonomy_from_quick_edit( $show, $taxonomy, $post_type ) {
		if ( Post_Types::CPT_ALERT === $post_type && in_array(
			$taxonomy,
			[
	...
	private function is_job_alert_admin_query( $query ) {
		return $query->is_main_query() && is_admin() && Post_Types::CPT_ALERT === $query->get( 'post_type' );
	}
