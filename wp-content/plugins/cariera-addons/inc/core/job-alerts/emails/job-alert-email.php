...
class Job_Alert_Email extends Email_Base {
	...
	public static function get_name() {
		return esc_html__( 'Job Alert E-mail', 'cariera-addons' );
	}
	...
	public static function get_description() {
		return esc_html__( 'Send regular e-mails with new job listings matching a search.', 'cariera-addons' );
	}
	...
	public function get_subject() {
		...
		return apply_filters( 'job_manager_alerts_subject', sprintf( __( 'Job Alert Results Matching "%s"', 'cariera-addons' ), $alert->get_name() ), $alert );
	}
	...
	public function is_valid() {
		...
		if ( ! isset( $args['alert'] ) || ! $args['alert'] instanceof \Cariera_Addons\Core\Job_Alerts\Alert ) {
			return false;
		}
		...
		if ( ! isset( $args['user'] ) || ! ( $args['user'] instanceof \WP_User || $args['user'] instanceof \WP_Job_Manager\Guest_User ) ) {
			return false;
		}
	...
	public function get_plain_content() {
		...
		if ( ! str_contains( $template, '{alert_page_url}' ) ) {
			$template .= "\n\n--\n\n" . __( 'Manage Alerts: {alert_page_url}', 'cariera-addons' );
			$template .= "\n" . __( 'Unsubscribe: {alert_unsubscribe_url}', 'cariera-addons' ) . "\n";
		}
	...
	public function get_rich_content() {
		...
		return \Cariera_Addons\Helpers::get_template(
			'alerts/emails/email-job-alert.php',
			[
	...
	private function get_jobs_content( $jobs ) {
		if ( ! $jobs || ! $jobs->have_posts() ) {
			return \Cariera_Addons\Helpers::get_template( 'alerts/emails/email-job-alert-no-jobs.php', [] );
		}

		return \Cariera_Addons\Helpers::get_template(
			'alerts/emails/email-job-alert-jobs.php',
			[
				'jobs' => $jobs->posts,
			]
		);
	...
	private function get_next_alert_date( $alert ) {
		$schedules = \Cariera_Addons\Core\Job_Alerts\Notifier::get_alert_schedules();
	...
	private function get_customizable_content() {
		...
		if ( ! $template ) {
			$template = \Cariera_Addons\Core\Job_Alerts\Settings::instance()->get_default_email();
		}
	...
	private function render_jobs_plaintext( $jobs ) {
		if ( ! $jobs || ! $jobs->have_posts() ) {
			return esc_html__( 'No jobs were found matching your search. Login to your account to change your alert criteria', 'cariera-addons' );
		}
		...
		while ( $jobs->have_posts() ) {
			...
			$job_content .= \Cariera_Addons\Helpers::get_template( 'alerts/content-email_job_listing.php', [] );
		}
	 ...
	 */
	private function get_replacements( $alert, $user ): array {
		...
		$alert_expiry = $expire_date ? sprintf( __( 'This job alert will automatically stop sending after %s.', 'cariera-addons' ), $expire_date ) : '';
		...
		$unsubscribe_url = add_query_arg(
			[
				...
				'alert_id' => $alert->ID,
				'user_id'  => $user->ID,
				'token'    => ( new \WP_Job_Manager\Access_Token( [ $alert->ID, $user->ID ] ) )->create(),
			],
			$manage_url
		);
		...
		if ( $user instanceof \WP_Job_Manager\Guest_User ) {
			...
			$manage_url = add_query_arg( [ \WP_Job_Manager\Guest_Session::QUERY_VAR => $token ], $manage_url );
