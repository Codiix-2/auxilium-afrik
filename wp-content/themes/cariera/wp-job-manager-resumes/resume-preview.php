...
<form method="post" id="resume_preview" action="<?php echo esc_url( $form->get_action() ); ?>">
	...
	<div class="job_listing_preview_title cariera-listing-submission">
		...
		<input type="submit" name="edit_resume" class="button" value="<?php esc_attr_e( '&larr; Edit resume', 'cariera' ); ?>" />
		<?php do_action( 'cariera_resume_submission_steps' ); ?>
		<input type="submit" name="continue" id="resume_preview_submit_button" class="button job-manager-button-submit-listing" value="<?php echo esc_attr( apply_filters( 'submit_resume_step_preview_submit_text', __( 'Submit Resume &rarr;', 'cariera' ) ) ); ?>" />
