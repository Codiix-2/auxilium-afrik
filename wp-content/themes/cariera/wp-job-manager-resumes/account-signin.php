...
if ( is_user_logged_in() ) { ?>
	...
	<fieldset>
		...
		<div class="field account-sign-in">
			...
			<?php } else { ?>
				<p><?php esc_html_e( 'You can proceed without an account, but if you already have one, feel free to log in.', 'cariera' ); ?></p>
			<?php } ?>
	...
	if ( ! empty( $registration_fields ) ) {
		...
		<div class="submit-listing-box submit-resume_registration-info">
			<h2 class="title"><?php esc_html_e( 'Registration Details', 'cariera' ); ?></h2>
			<div class="form-fields">
				<?php
				foreach ( $registration_fields as $key => $field ) {
					?>
					<fieldset class="fieldset-<?php echo esc_attr( $key ); ?>">
					<label for="<?php echo esc_attr( $key ); ?>"><?php echo esc_html( $field['label'] ) . wp_kses_post( apply_filters( 'submit_resume_form_required_label', $field['required'] ? '' : ' <small>' . esc_html__( '(optional)', 'cariera' ) . '</small>', $field ) ); ?></label>
