...
global $company_preview, $listing_half_detail;

if ( $company_preview ) {
	return;
}

if ( $listing_half_detail ) {
	return;
}
...
$active_job_listings       = cariera_get_the_company_job_listing_active_count( $post->ID, true );
$jobs_page_url             = job_manager_get_permalink( 'jobs' );
$companies_search_page_url = ! empty( $jobs_page_url ) ? add_query_arg( 'search_companies', "{$post->ID}", $jobs_page_url ) : false;

if ( $companies->have_posts() ) {
	do_action( 'cariera_company_job_listings_before' ); ?>

	<div id="company-job-listings" class="company-job-listings">
		<h2 class="content-title"><?php esc_html_e( 'Open Job Positions', 'cariera' ); ?>
