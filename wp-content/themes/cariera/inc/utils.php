...
require_once locate_template( 'inc/utils/templates.php' );
require_once locate_template( 'inc/utils/theme-functions.php' );
...
/**
 * Check if Cariera Company Events Manager is activated.
 *
 * @since 1.8.2
 */
function events_manager_is_activated() {
	return class_exists( '\Cariera_Events' ) ? true : false;
}
...
function is_elementor_ajax() {
	return ! empty( $_REQUEST['_nonce'] ) && wp_verify_nonce( wp_unslash( $_REQUEST['_nonce'] ), 'elementor_ajax' ); // phpcs:ignore
}

/**
 * Check if Query Monitor is activated.
 *
 * @since 1.9.0
 */
function is_qm_activated() {
