<?php
/**
 * Custom: Job Listing - Grid Version 1
 *
 * This template can be overridden by copying it to yourtheme/job_manager/job-template/content-job_listing_grid1.php.
 *
...
$job_class = 'job-grid single_job_listing_1';
	...
	<div class="job-content-wrapper">
			...
			<div class="job-company <?php echo ! empty( $cover_img ) ? esc_attr( 'with-bg' ) : ''; ?>" <?php echo ! empty( $cover_img ) ? 'style="background-image: url(' . esc_attr( $cover_img ) . ')"' : ''; ?>>
				...
				<div class="job-cat">
					...
					if ( ! empty( $job_categories ) && is_array( $job_categories ) ) {
						echo '<span>' . esc_html( $job_categories[0]->name ) . '</span>';
					}
					?>
				</div>
			</div> 
		...
		<div class="listing-body">
			...
			<div class="job-info">
				...
				<?php if ( $company ) { ?>
					<span class="company">
						<strong><?php esc_html_e( 'Company: ', 'cariera' ); ?></strong>
						<?php the_company_name(); ?>
					</span>
				<?php } ?>

				<span class="location">
					<strong><?php esc_html_e( 'Location: ', 'cariera' ); ?></strong>
				...
				if ( $rate_min ) {
					...
					?>
					<span class="rate">
						<strong><?php esc_html_e( 'Rate: ', 'cariera' ); ?></strong>
				...
				if ( $salary_min ) {
					...
					?>
					<span class="salary">
						<strong><?php esc_html_e( 'Salary: ', 'cariera' ); ?></strong>
			...
			<div class="job-actions">
				<a href="<?php the_job_permalink(); ?>" class="btn btn-main btn-effect"><?php esc_html_e( 'view details', 'cariera' ); ?></a>
				<a href="#quickview" class="job-quickview" data-id="<?php echo esc_attr( $job_id ); ?>"><i class="lar la-eye"></i></a>
			</div>
		</div>
	</div>
</li>
