...
<form action="<?php echo esc_url( $action ); ?>" method="post" id="submit-job-form" class="job-manager-form" enctype="multipart/form-data">
	...
	if ( isset( $resume_edit ) && $resume_edit ) {
		printf( '<p><strong>' . esc_html__( 'You are editing an existing job. %s', 'cariera' ) . '</strong></p>', '<a href="?job_manager_form=submit-job&new=1&key=' . esc_attr( $resume_edit ) . '">' . esc_html__( 'Create A New Job', 'cariera' ) . '</a>' );
	}
	?>
	...
	<?php if ( job_manager_user_can_post_job() || job_manager_user_can_edit_job( $job_id ) ) : ?>
		...
		<div class="cariera-listing-submission">
			...
			if ( isset( $can_continue_later ) && $can_continue_later ) {
				echo '<input type="submit" name="save_draft" class="button secondary save_draft" value="' . esc_attr__( 'Save Draft', 'cariera' ) . '" formnovalidate />';
			}
			?>
			...
			<input type="submit" name="submit_job" class="button" <?php if ( 'v3' === $captcha_version ) { echo 'onclick="jm_job_submit_click(event)"'; } ?> value="<?php echo esc_attr( $submit_button_text ); ?>" />
		</div>
