...
<div class="listing-submitted">
	...
	switch ( $job->post_status ) :
		case 'publish' :
			$job_submitted_content = '<div class="job-submitted">' . wp_kses_post(
				sprintf(
					// translators: %1$s is the job listing post type name, %2$s is the job listing URL.
					__( '%1$s listed successfully. To view your listing <a href="%2$s">click here</a>.', 'cariera' ),
			...
			if ( $job_dashboard_link && empty( $job_dashboard_title ) ) {
				$job_submitted_content .= wp_kses_post(
					sprintf(
						...
						__( '  <a href="%1$s"> View your %2$s</a>', 'cariera' ),
						$job_dashboard_link,
		...
		default :
			...
			$job_submitted_content = '<div class="job-submitted">' . wp_kses_post(
				...
				// translators: %1$s is the job listing post type name.
					__( '%1$s submitted successfully.', 'cariera' ),
					esc_html( $wp_post_types[\WP_Job_Manager_Post_Types::PT_LISTING]->labels->singular_name )
				)
			) . '</div>';
